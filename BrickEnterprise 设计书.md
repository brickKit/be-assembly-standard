...
| 初始化 | ⚠️ **平台不建库、不建 schema、不建 role**（`006` §9.5：建库要 `CREATEDB` 权限，让每个组件的运行期账号都有它是全平台提权；且 PG 不能在一个库内部创建它自己）。`brickkit up` 只会**打印**出本次需要预先创建的库和建库语句。<br>实际动作：由 `be-ops` 产出建置脚本（每组件 `CREATE SCHEMA` / `CREATE ROLE` / 授权 + 5 个外壳登录角色），运维**执行一次**。<br>⚠️ **但「平台不建库」不等于「不能自动建库」——那是平台的约束，不是我们的。** 我们自己的工具链会做，只是必须在部署文档里逐层写清「哪些自动、哪些要人执行一次」。落点分两处：**① `brickkit_db` 与 `casdoor`/`keycloak` 两个非组件 schema 由 `infra/postgres/initdb/` 在 PostgreSQL 首次启动时自动建**（带外容器在 `be-ops` 就绪之前就要连库）；**② 62 个组件的 schema/role 归 `be-ops` 产出 2**。⚠️ 前者只在数据卷为空时跑一次，所以**后者必须全程 `IF NOT EXISTS`**，两者共存不冲突。**给部署人员的那张五层表在 [`docs/ops/zh/deployment-handbook.md`](docs/ops/zh/deployment-handbook.md) §3。**<br>⚠️ **另有一件容易漏的**：`brickkit.yaml` 里每个资源的 `bindings` **必须逐组件列出 `componentId`**——声明了资源依赖却没绑定，`up` 会阻断（`006` §4.4）。合并态下这意味着 **5 条 PostgreSQL 资源条目**（同 host、5 个外壳登录角色的不同凭据），每条带着该外壳内组件的 bindings。`DATABASE_NAME` 注入的是 binding 上的 `database` 字段，全系统都写 `brickkit_db`；**schema 不走 binding**，走组件自己 `configSchema` 的 `pgSchema` |
...
**各域该看哪个项目的哪一块**，以及各自要避开什么，见装配仓库 `docs/standards/zh/00-master-guide.md` §4 的 **SOP-R**（那份表会随开工进度补充，放在本书里会立刻过期）。
...
⚠️ **推论（阶段一实测才第一次真的踩到）：这条"没有事后补救"对普通 Docker 部署下的组件同样成立，不只是 `local: true` 之间会撞车这一种表现。** 一个从没设过 `local: true`、就用 `brickkit up` 正常起在 Docker 里的组件，如果想让宿主机上的 `grpcurl` 连它的 gRPC 端口——`expose: true`/`exposePort`（§6.3.1，只映射 `deployment.port` 那一个主端口）帮不上忙，`localPort`（本节，只在 `local: true` 语境下存在）也帮不上忙。**平台没有给"从宿主机直接访问一个普通组件的额外端口"留任何机制**，唯一路径是拿容器在 brickKit 生成的 Docker 网络里的 IP 直连（`docker inspect <容器名>` 查）。写验收脚本或本地联调需要连 gRPC 端口时，认这条路，不要去 `brickkit.yaml`/`component.yaml` 里找一个不存在的开关。
...
5. **UI 层逐格锁定，不许自选**（§12.6）：PC 用 **Ant Design Vue v4 + vxe-table**，移动端用 **wot-design-uni**（AntDV 依赖 DOM，在 Uni-app 里用不了），图表用 **ECharts**，应用骨架**自研**。第三方组件只许出现在 `packages/ui-kit` 里——**业务页面严禁直接 `import` 第三方**，判据见 §12.6.3。
...
| 功能 | 现实中的分歧 | 结论 |
| --- | --- | --- |
| 成本核算 | 移动加权平均 / 先进先出 / 标准成本 + 差异 / 分批实际成本。四种在不同行业都是正解，且**客户的会计师往往指定其中一种** | ~~真实的槽位族信号。`slot:costing`~~ **阶段二用真实调研验证后更正**：分歧真实存在，但长在被强依赖的 `erp-inventory` 内部，不能做成 slot——落点是 `customer_fork`。见下文 |
| 库存拣货策略 | 先到期先出 / FIFO / 就近库位 / 波次拣货。制造业与零售业的最优解不同 | ~~槽位族信号~~ **阶段二用真实调研验证后更正**：同上，不能做成 slot，落点是 `customer_fork`。见下文 |
| 审批路由 | 组织层级逐级 / 金额分级 / 矩阵式（职能线 + 行政线）/ 规则引擎 | ~~槽位族信号，但要先确认它不违反 §6.6~~ **阶段三写 `infra-workflow` 设计计划时判掉了：不能做成 slot。** 分歧真实存在，但它**根本不在 `infra-workflow` 里**——按 §6.6 铁律一，"这张单该谁审"由业务组件算好后通过 `assignee` 传入。所以它长在 `erp-sales`/`erp-purchase` 这类业务组件内部，而它们都被 `crm-*`/BFF 建了强依赖边 → 落点是 **`customer_fork`**，与本节下方那 9 条的判定一致 |
| 单据编号规则 | 流水号 / 按年月分段 / 按组织+类型分段 | **不是**槽位族——这是一个配置项，不值得一个组件 |
| 客户主数据的字段集 | 各家差别很大，但差别都在"多几个字段少几个字段" | **不是**槽位族——那是 `customer_fork` 的活（§3.4） |

⚠️ **阶段二用真实参考实现验证过这条判据，结果纠正了上表两行的预判，并补出一条比表里任何一行都更重要的结论。**

实现 `erp-inventory`/`erp-finance`/`erp-sales`（阶段二 · 验平台）时，SOP-R 的参考调研（详见三份组件设计计划各自的 §8）里，真的按本节判据找出了 **9 处"多种实现都合理，只是适配客户不同"的分歧**：

| 分歧 | 长在哪个组件 | 现实中的分歧 |
|---|---|---|
| 成本核算法 | `erp-inventory` | ERPNext 按物料配 / Odoo 按产品类别配 |
| 补录历史的策略 | `erp-inventory` | ERPNext 改写历史（repost）/ Odoo 追加冲销层（vacuum） |
| 拣货/发料策略 | `erp-inventory` | Odoo 参数化 FIFO/LIFO/FEFO/就近/整包优先 |
| 关账策略 | `erp-finance` | Tryton 期间实体 / Odoo 公司级 lock date / ERPNext 三套并存 |
| 迟到单据策略 | `erp-finance` | 拒绝 / 顺延下期 / 有权限角色强改 |
| 预留严格度 | `erp-sales` | Odoo 配置项 `reservation_method` |
| 缺货策略 | `erp-sales` | Odoo 配置项 `create_backorder` + `picking_policy` |
| 开票基准 | `erp-sales` | Odoo 配置项 `invoice_policy` |
| 定价引擎形态 | `erp-sales` | Odoo 有序表第一条赢 / metasfresh 规则链可组合 |

按本节判据**第一步**（"每一种都合理，只是适配客户不同"）逐条检验，这 9 条全部成立；但按**第三步的硬约束**（"没有任何组件对它建依赖边"）再检验一遍，9 条**全部不成立**——三个组件全都被 `erp-sales` 或未来的 `crm-*`/BFF 建了强依赖边。**9 条一个都不能做成 slot。**

**这暴露了本节判据原来没写清楚的一点："每一种都合理"是槽位族的必要条件，不是充分条件；硬约束（无依赖边）必须同时满足。** 只看第一步、不看第三步，正是上表"成本核算""库存拣货策略"两行原判断错误的原因——两条分歧都真实存在，只是长错了地方。
...
⚠️ **严禁的做法：在一个组件里写 `if costingMethod == "fifo"`。** 那等于把"多客户的不同需求"塞进一个仓库，最后每个客户的需求都在同一份代码里互相牵制——正是第 1 章说的"牵一发而动全身"。**这一条的例外只有一种：分歧小到只是几个分支、且看得见的未来不会长出第五种**（判据见 `docs/standards/zh/03-ai-development-standard.md` §1.3）。
...
- **第三方 UI 组件只许出现在 `packages/ui-kit-pc` 与 `packages/ui-kit-mobile` 里**，业务页面一律用我们自己的组件名（§12.6.3）。两端**用不同的组件库是设计使然**——移动端的界面形态本来就和 PC 端完全不一样（§12.6.1.1），所以两端唯一共享的 UI 资产是 **`packages/design-tokens`**（纯数据、零框架 import：主色、间距、字号、行高、圆角）。**共享 token，不共享组件。**
- `packages/ui-kit-pc` 另有两件一次性的接线活（§12.6.2）：① 把 Ant Design Vue 的 design token 派生成 vxe-table 的 CSS 变量，**主题只有一处真相源**；② 把 AntDV 的表单控件注册成 vxe-table 的全局单元格渲染器。**不做这两件的症状是不报错、能跑，只是每张 ERP 表格看起来像贴进页面里的。**
...
**端口**：HTTP `8223` / gRPC `9223`（⚠️ 8222 是 NATS 的监控端口，跳过）。**schema**：`infra_authz`。**外壳三**。
...
| 119 | **精确版本锁的是"不许写范围"（`^1.2` 不行），不是"必须追最新"**：能自由选的依赖，默认贴着系统已经有的版本走，只有某个版本带来确实要用的功能时才升级；被依赖链动态顶上去的（如 `be-sdk-go` 因 gin/grpc/otel/prometheus/nats.go 最新版全要求 `go>=1.25` 而定为 `go 1.25`），跟着走，不为了凑一个旧数字去回退换取过时依赖 | 反例是本项目自己踩过的：PostgreSQL 一开始选 `16-alpine`，而本机早就装着 `postgres:15`，翻遍设计书找不出一条"非 16 不可"的理由——纯粹是选的时候没有对照系统现状。⚠️ **但发现这条时 `be-postgres` 已经用 16 建过库，切回 15 需要真实的数据迁移**（实测：直接换镜像会报「database files are incompatible with server」）——本条判据管的是**以后新选**版本时怎么定，不是把已经生效、要花真代价才能改的选择倒回去。已经在跑的维持 16 | ① 挑版本号时凭"新的应该更好"的直觉，而不是先问"系统已经有什么、够不够用"；② 反过来把"精确版本"误读成"必须锁定某个具体旧版本不许动"，导致依赖链需要更高版本时也不敢升，实际上卡住的是安全补丁与 bug 修复 |
...
> ⚠️ **本节只定语言与框架，不定组件库。** 上面这几个库在这里只是「Vue 生态足够好」的证据。**实际锁定哪一个见 §12.6**（结论：PC 用 Ant Design Vue + vxe-table，移动端用 wot-design-uni，Element Plus 不用）。
...
⚠️ **Gin 这一格是纪律锁（第③层），不是物理锁——必须说清楚**，否则将来有人以为它和 FastAPI 一样是硬约束，或者反过来以为 FastAPI 也只是偏好。`gin.Engine` 本身就是 `http.Handler`，Gin + Echo + chi 塞进一个 Go 进程能编译能跑。锁它的理由是三条：① `be-sdk-go` 的中间件（OTel、request-id、error → gRPC status、PII 脱敏日志、RED 指标）只写一遍——写第二遍那份必然烂；② 外壳的 `go.work` 不用同时拖两套框架；③ **新开会话的 AI 读两个组件看到两套写法，跨组件抄一段就编译不过**（`docs/standards/zh/03-ai-development-standard.md` §1.1：帮 AI 的是一致与显式）。
...
| 层 | PC（`apps/pc-web`） | 移动端（`apps/mobile`，Uni-app） |
| --- | --- | --- |
| 组件库 | **Ant Design Vue v4** | **wot-design-uni**（默认，退路 `uni-ui`）。**两端用不同库是设计使然，不是妥协**——见 §12.6.1.1 |
| 表格 | **vxe-table**（可编辑单元格、虚拟滚动、树表格、冻结列——AntDV 自带的 Table 撑不住 ERP 的表格） | 不用表格。移动端是卡片列表 + 下拉刷新 |
| 图表 | **ECharts + `vue-echarts`**（Apache-2.0，**离线可用**——本地化部署没有外网 CDN） | 同左（Uni-app 侧走 canvas 渲染） |
| 应用骨架 | **自研**（§12.6.4） | 自研 |
| 视觉方向 | **克制专业型**（§12.6.5） | 同左 |

⚠️ **精确版本**，与平台铁律一致：`package.json` 里不许出现 `^` / `~` / `latest`。`vxe-table` 与 `vxe-pc-ui` 有配套关系，把当次锁定的组合写进 `packages/ui-kit-pc/README.md`——**不要写进本书**，那种版本号会过期。

#### 12.6.1.1 两端用不同组件库，是设计使然

**先把因果说对，否则后面会有人想「统一」它。** 不是「本来想两端共用一套、被 DOM 挡住了」，而是**移动端的界面本身就和 PC 端完全不一样**：

| | PC（`apps/pc-web`） | 移动端（`apps/mobile`） |
| --- | --- | --- |
| 主要形态 | 密集表格、多列筛选区、多级联动长表单、驾驶舱 | 卡片列表、下拉刷新、扫码、拍照上传、审批通过/驳回两个按钮 |
| 交互 | 鼠标 + 键盘（表格里连续录入、Tab 跳格） | 手指（点击热区、滑动、手势） |
| 一屏信息量 | 越多越好 | 越少越好 |
| 典型岗位 | 财务、仓管、单据录入员——全天盯着屏幕 | 销售、外勤、审批人——路上用几十秒 |

**两端共用一套组件库的收益本来就接近零**，因为几乎没有一个组件能原样两端复用（表格没法用，筛选区要折叠成筛选页，长表单要拆成分步）。而 AntDV 依赖 DOM、Uni-app 编到小程序 / App 时没有 DOM，只是**顺便把「偷懒复用」这条路物理封死了**——它是结论的加固，不是结论的理由。

**连带的结构后果：`packages/ui-kit` 不能是一个包。** 一个共享的 ui-kit 只要 `import` 了 `ant-design-vue`，Uni-app 那侧就会把 AntDV 一起打进去（小程序端很可能直接编不过，因为 AntDV 在模块顶层就摸 DOM API）。所以拆三个：

| 包 | 内容 | 谁用 |
| --- | --- | --- |
| **`packages/design-tokens/`** | **纯数据、零框架 import**：主色、色阶规则、间距梯度、字号、行高、圆角。**这是两端唯一共享的 UI 资产** | 两端都用 |
| `packages/ui-kit-pc/` | AntDV + vxe-table 的封装与接线（§12.6.2）；第三方 PC 组件的唯一出口 | 只有 `apps/pc-web` |
| `packages/ui-kit-mobile/` | wot-design-uni 的封装；第三方移动端组件的唯一出口 | 只有 `apps/mobile` |

⚠️ **「好看 = 一致」（§12.6.5）在这个拆法下的落点是：共享 token，不共享组件。** 两端看起来是一家的产品，靠的是同一份 `design-tokens`，而不是同一批组件——**反过来做（为了一致去共用组件）会同时得到一个难用的移动端和一个被移动端限制住的 PC 端**。

#### 12.6.2 ⚠️ AntDV 与 vxe-table 是两套主题系统，必须在 `ui-kit-pc` 里接起来

它们各管一套。**没人接起来的症状是：不报错、能跑，只是每张 ERP 表格看起来像贴进页面里的**——主色不一样、边框不一样、行高不一样，而可编辑单元格里的输入框是 vxe 的而不是 AntDV 的。**这是本节唯一一条会直接毁掉观感的。**

| | Ant Design Vue v4 | vxe-table |
| --- | --- | --- |
| 主题入口 | `<a-config-provider :theme="{ token, algorithm }">` | `VxeUI.setTheme('light' \| 'dark')` |
| 变量形态 | design token（seed → map → alias 三级派生） | CSS 变量（`--vxe-ui-font-primary-color` 等） |
| 派生工具 | `theme.darkAlgorithm` / `theme.compactAlgorithm` | `VxeUI.color.lighten()` / `VxeUI.color.darken()` |

两件一次性的活，都在 `packages/ui-kit-pc`，各只写一次（源头的 token 值来自 `packages/design-tokens`，见 §12.6.1.1）：

1. **token 只有一处真相源。** AntDV 的 seed token 是源；启动时把它派生成 `--vxe-ui-*` 写到 `document.documentElement`（vxe 自己就提供 `VxeUI.color.lighten/darken`，等于它承认要这么干）。**暗色 / 紧凑的开关也只有一处**——`theme.algorithm` 与 `VxeUI.setTheme()` 必须在同一个函数里切。分两处切，必然有一天只切了一半。
2. **可编辑单元格里的控件一律用 AntDV。** vxe 的 `:edit-render="{}"` + `#edit` 插槽里放什么组件都行，所以放 `<a-select>` / `<a-input-number>` / `<a-date-picker>`。**注册一次成全局渲染器**，业务页面不再重复写。

顺带一个免费收益：**`compactAlgorithm`**。ERP 一屏要塞更多行，用 AntDV 自带的紧凑算法比在每个业务页面里手调 `padding` 靠得住得多。

#### 12.6.3 混搭第三方组件的四条判据

「需要时可以混搭」不带判据的话，一年后一个页面里会有五个库。

1. **只在 AntDV 确实没有这个能力时才混。** 已知的合法缺口：甘特图（`prj-project`）、审批流设计器（`infra-workflow`）、富文本、代码编辑器、大屏可视化。
2. **混进来的必须先进 `packages/ui-kit-pc` 或 `ui-kit-mobile` 包一层**，对外暴露我们自己的组件名。**业务页面严禁直接 `import` 第三方**——与铁律六同一个道理：换实现时改一个文件，而不是改 40 个页面。⚠️ 表格是唯一一个薄封装做不到全保真的组件，它的规矩单列在 §12.6.6 第 4 步。
3. **每混一个，在 `frontend-*` 的设计计划第 8 节记一行**：哪个能力、为什么 AntDV 不行、许可证。
4. ❌ **不许为了「好看」混。** 好看靠 token + 密度 + 排版 + 留白，不靠换组件库。**两个库的 token 体系对不齐，混得越多越不好看**——这一条与第 1 条方向相反，所以必须写明：能力缺口是混的唯一理由，观感不是。

#### 12.6.4 应用骨架：自研，但不从零发明

**自研。** 理由是我们有两条约束恰好落在现成 admin 模板**最难改的那一层**：

| 我们的约束 | 为什么模板改不动 |
| --- | --- |
| **动态特性路由**（§5.9 铁律 5：启动时调 `GET /api/tenant/features`，按启用清单动态注册路由与菜单） | 模板的「路由 + 权限 + 菜单」是嵌得最深的一层，通常由后端菜单表或「静态路由 + 角色码」驱动。改成 feature-flag 驱动等于重写那一层——**而那一层正是模板的主要价值**，重写完就只剩一个 layout |
| **Uni-app 共享层**（`packages/` 给 `apps/pc-web` 与 `apps/mobile` 共用） | 没有任何 admin 模板覆盖 Uni-app，这一层无论如何是我们自己的 |
...
ERP 用户要的是**熟悉感与信息密度**，不是惊喜。方向定为「AntDV 默认为底，只在 token 层做克制的调整」：

| 项 | 定法 |
| --- | --- |
| 颜色 | **只改 seed token 的主色**，其余全部走 AntDV 的派生。业务页面里**禁止出现硬编码颜色值** |
| 密度 | 开 `compactAlgorithm`；表格行高由 `ui-kit` 统一给，不在页面里调 |
| 圆角 / 阴影 | 小圆角、克制阴影。**不做卡片包裹一切** |
...
vxe 的文档里带 `enterprise-version` / `enterprise-link` 这类标记，说明**部分能力属于商业版**，而具体哪些落在免费档**本书没有核实**（写作时未核，不要照抄本节当结论）。

**所以 vxe-table 这一格的状态是「默认选择，待核」，不是「已锁死」。** 处理方式是一个闭环，不是一句提醒：

**第 1 步 · 开工前逐条核（`frontend-standard` 的第一件事）**

按我们真要用的能力列清单去核，一条一条对着官方的版本矩阵打勾：

| 要核的能力 | 谁需要它 |
| --- | --- |
| 虚拟滚动（万行不卡） | 所有列表页；§11.4 的时间窗口只能缩小数据量，不能保证行数小 |
| 冻结列 + 固定表头 | 单据明细（左边固定行号/物料，右边横向滚十几列） |
| 可编辑单元格 + 键盘连续录入（Tab 跳格） | 单据录入——**这是 ERP 表格与普通表格的分水岭**，也是最可能落在付费档的一类 |
| 树表格 | BOM 展开、组织树、科目表、产品分类 |
| 导入 / 导出 Excel | 几乎每个列表页客户都会要 |
| 透视 / 分组汇总 | `ana-bi`、财务报表 |
| 甘特图 | `prj-project`（也可能由 `vxe-pc-ui` 提供，同样要核档位） |

**第 2 步 · 判定**

| 核出来的结果 | 怎么办 |
| --- | --- |
| 全部在免费档 | 锁定 vxe-table，把版本与核对日期写进设计计划第 8 节 |
| 少数落在付费档，且**不是分水岭那几条** | 记下来，用免费档能力绕，绕法写进设计计划 |
| **可编辑单元格 / 虚拟滚动 / 冻结列**里有落在付费档的 | **换**。这三条是 ERP 表格的地板，绕不过去 |
| 买 | 也是一条路，但要先算清**私有化交付时每个客户是否都要授权**——这决定它是一次性成本还是每单成本 |

**第 3 步 · 换的话，候选与同一个陷阱**

⚠️ **所有严肃的表格库都把「企业能力」做成付费档**，所以候选**一律要核同一份清单**，不要以为换一个就没这个问题：

| 候选 | 免费档大致覆盖 | 代价 |
| --- | --- | --- |
| **AG Grid Community**（MIT） | 虚拟滚动、编辑、排序筛选、冻结列 | 分组 / 透视 / 主从明细在 Enterprise（付费）。Vue 封装是官方件但文档以 React/JS 为主，AI 语料偏 React |
| **RevoGrid**（MIT，无付费档） | 虚拟滚动、编辑、冻结列 | 组件生态与 ToB 细节（右键菜单、工具栏、导出）要自己补；语料少 |
| **TanStack Table**（MIT，headless，无付费档） | 只给状态与逻辑，**渲染全是我们的** | 表格 UX 从零写——冻结列、虚拟滚动、键盘导航都要自己实现。**只在前两条都不行时才考虑** |

**第 4 步 · 把换的代价提前压低（现在就要做的一件事）**

`packages/ui-kit-pc` 必须从第一天就导出**我们自己的 `<BeTable>`**，而不是让页面直接用 `<vxe-grid>`。这样换引擎是改一个包，不是改 40 个页面。

⚠️ **但这里要诚实：表格是唯一一个「薄封装」不可能做到全保真的组件**——它的 API 面太大（列定义、插槽、编辑器、事件、实例方法）。所以规矩定成可执行的两条，而不是一句空话：

1. `<BeTable>` 只封**80% 的用法**：列定义、数据源、编辑、选择、导出、分页/虚拟滚动开关。
...
**TTL 定值：access token 10 分钟；refresh token 12 小时，每次刷新轮换（rotation），不做滑动续期。**

有了 `stale_since`，TTL 已经不是生效路径，它现在只决定一件事：**authz 挂掉、组件跑在旧 bundle 上时，一个已被撤权的人还能撑多久**（§14.1.9 的 fail-static 爆炸半径）。所以取短的一头。

代价小到可以忽略：500 人在线、10 分钟 TTL ≈ **0.83 QPS** 的 refresh，本地化单机上等于没有。refresh 取 12 小时是「**一个班次登录一次**」——仓管用共享终端时这反而是对的，而且不用维护滑动续期的绝对上限那套账。

⚠️ **改这两个值之前先想清楚改的是什么**：调短 access 只在「authz 长时间不可达」这一种情形下有收益；调长它会同比放大那个爆炸半径，而**换不来任何生效速度**——生效速度由 bundle 轮询间隔（15s）决定，与 TTL 无关。
...
#### 14.1.10 字段级权限（`type: field`）的掩码：手写，不上机制

真正需要它的只有三个组件、五个字段：`hrm-payroll-*` 的薪资、`erp-finance` 的金额、`mdm-product` 的成本价。**所以不引入任何机制**：

| 方案 | 为什么不选 |
| --- | --- |
| 反射 + struct tag | DTO 是 **protobuf 生成的结构体**，加自定义 tag 要额外的 protoc 插件；而且列表接口上每行每字段一次反射，成本随行数线性涨 |
| 代码生成 | 直接撞决策 32（**不为代码生成自研工具链**） |
| **手写** ✅ | 三个组件、五个字段。**为它引入任何抽象，成本都超过重复成本**（SOP-P：逻辑本身简单却硬套模式是更糟的结果） |

**写法（三个组件长成同一个样子）**：每个组件一个 `mask.go`，一个 DTO 一个函数，在 handler 出口、序列化之前调用。

```go
// mask.go —— 本组件全部字段级掩码集中在这一个文件里
func maskProduct(p *pb.Product, perms besdk.PermSet) *pb.Product {
    if !perms.Has("mdm.product.field.cost") {
        p.CostPrice = ""      // 金额字段是 string（decimal 跨语言丢精度）
    }
    return p
}
```

⚠️ **掩码是「置空」，不是「删字段」。** 契约只能追加、不能变形（§3.4 铁律 3）——删字段会让生成的 TypeScript 类型与实际响应对不上。前端拿同一个权限键决定渲染值还是渲染「——」。

⚠️ **守它的是测试，不是门禁。** 每个 `type: field` 的权限键必须有一条 **L2 测试，同时覆盖单条与列表两条路径**。
> **「单条掩了、列表忘了掩」是这里唯一真正会发生的错**——单条详情页测过了、看起来一切正常，而列表接口把成本价原样吐了出去。AST 级的门禁扫不出这个（它判不了「哪个 handler 该调哪个 mask 函数」），一条强制测试可以。
...
| 字段级掩码只在单条详情做了，列表接口忘了 | 详情页测过、看起来一切正常，而**列表接口把成本价/薪资原样吐出去**。§14.1.10 要求每个 `field` 键的 L2 测试必须同时覆盖两条路径 |
