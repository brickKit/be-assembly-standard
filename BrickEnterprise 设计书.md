...
- **`packages/ui-kit` 是这个仓库里唯一允许出现第三方 UI 组件的地方**，它承担三件事（§12.6）：① 把 Ant Design Vue 的 design token 派生成 vxe-table 的 CSS 变量，**主题只有一处真相源**；② 把 AntDV 的表单控件注册成 vxe-table 的全局单元格渲染器；③ 导出间距 / 字号 / 行高常量，供业务页面引用而不是各写各的。**不做这三件事的症状是不报错、能跑，只是每张 ERP 表格看起来像贴进页面里的。**
...
| 111 | **前端 UI 层逐格锁定**（§12.6）：PC = **Ant Design Vue v4 + vxe-table**，移动端 = **wot-design-uni**，图表 = **ECharts**；第三方组件**只许出现在 `packages/ui-kit`**，业务页面严禁直接 `import`；混搭的唯一理由是**能力缺口**，不是观感 | AntDV 与 vxe-table 是**两套主题系统**（一套 design token、一套 CSS 变量），不在 `ui-kit` 里接起来就会「每张表格像贴进去的」——而这一条不报错、能跑。移动端那一格是**物理的**：AntDV 依赖 DOM，Uni-app 编到小程序 / App 时没有 DOM。`ui-kit` 唯一出口那一条与 `be-sdk-*` 同一个道理：换实现改一个文件，而不是改 40 个页面 | ① 一年后一个页面里五个 UI 库，token 体系两两对不齐，**混得越多越不好看**；② 为了「好看」去换组件库，而真正毁观感的是「这一页间距 16、下一页 20」；③ 页面设计完了才发现 vxe 的某个能力属于商业版（§12.6.6） |
...
| --- | --- | --- |
| 组件库 | **Ant Design Vue v4** | **wot-design-uni**。⚠️ AntDV 在这里**用不了**——它依赖 DOM，Uni-app 编到小程序 / App 时没有 DOM。这是两端唯一一处必须用不同库的地方 |
...
⚠️ **精确版本**，与平台铁律一致：`package.json` 里不许出现 `^` / `~` / `latest`。`vxe-table` 与 `vxe-pc-ui` 有配套关系，把当次锁定的组合写进 `packages/ui-kit/README.md`——**不要写进本书**，那种版本号会过期。

#### 12.6.2 ⚠️ AntDV 与 vxe-table 是两套主题系统，必须在 `ui-kit` 里接起来
...
两件一次性的活，都在 `packages/ui-kit`，各只写一次：
...
2. **混进来的必须先进 `packages/ui-kit` 包一层**，对外暴露我们自己的组件名。**业务页面严禁直接 `import` 第三方**——与铁律六同一个道理：换实现时改一个文件，而不是改 40 个页面。
...
⚠️ **「好看」在这个项目里的具体含义是「一致」。** 61 个组件的页面由 AI 分批生成，最容易毁掉观感的不是配色不够大胆，而是**这一页的间距是 16、下一页是 20**。所以 `ui-kit` 必须导出间距 / 字号 / 行高的常量，而页面只许引用它们。**这一条比选哪个组件库重要。**

#### 12.6.6 ⚠️ vxe-table 的商业版边界，开工前必须逐条核

vxe 的文档里带 `enterprise-version` / `enterprise-link` 这类标记，说明**部分能力属于商业版**。我们是私有化交付，而设计计划模板第 8 节本来就有「许可证（已复核）」这一列。

所以：**在 `frontend-standard` 开工前，逐个核对真要用的能力在不在免费档**——虚拟滚动、树表格、可编辑单元格、导入导出、透视表、Gantt。结论写进 `docs/design/frontend-standard.md` 第 8 节。

**别把页面设计完了才发现某个能力要买。** 这与端口册那条时序性是同一类风险：**发现得越晚越改不动**——等 40 个页面都围着某个能力设计完了，换掉它就是重画。
