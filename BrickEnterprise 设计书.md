...
#### 3.2.1 参考实现纪律：先看现实里的 ERP 怎么做，但绝不抄 copyleft 的代码

**每个组件的业务逻辑都必须先去看现实中的开源 ERP/CRM 怎么做的。** 理由不是谦虚：ERP 的领域模型是几十年踩坑踩出来的，凭空设计出来的「订单状态机」「库存流水」「会计凭证」几乎必然漏掉边界情形——而那些边界情形要到客户上线三个月后才暴露。

这也是每个组件的设计计划必须写明「看了哪个项目的哪个模块」的真正原因：它不只是给人看的出处，更是**给 AI 的源码指针**——写不出某段逻辑时先去读那个模块，再回来实现。

**⚠️ 但这里有一条会让整套交付模型在法律上失效的红线。**

本节的模型是：`open_standard` 标准件 100% 开源 → 客户需要定制时 Fork → **`customer_fork` 闭源且归客户所有**。而主流开源 ERP 几乎全是 copyleft（Odoo 是 LGPL-3，ERPNext / Tryton / Dolibarr / EspoCRM 是 GPL-3，SuiteCRM / Twenty 是 AGPL-3）。**把 copyleft 代码复制进标准件，客户 Fork 件就是衍生作品、必须同样开源——「闭源且归客户所有」当场失效。**

| # | 纪律 | 说明 |
| --- | --- | --- |
| **一** | **读设计、领域模型、状态机、字段设计——永远安全** | 学「库存流水为什么必须不可变」「会计期间为什么要锁」「UoM 换算因子放哪张表」，这是知识不是代码。**主要就靠这一条** |
| **二** | **复制代码只限许可证宽松的项目**（Apache-2.0 / MIT / BSD），且必须在 `NOTICE` 里保留原始版权与许可证声明，并在设计计划里写明抄了哪个文件的哪一段 | 大型 ERP 里**只有 Apache OFBiz 是 Apache-2.0** |
| **三** | **严禁把 GPL / LGPL / AGPL 项目的代码复制、翻译、或逐行改写进任何组件** | **「用 Go 重写一遍 Odoo 的那个函数」同样是衍生作品。** 这是最容易自我说服的一条——「我只是参考了它的算法」和「我把它翻译成了 Go」中间那条线，法务上比直觉里窄得多 |

⚠️ **许可证以各项目仓库当前的 `LICENSE` 文件为准，开工前现场复核。** 开源项目改许可证不是罕见事，任何文档里的许可证标注都只是写作时的记录，不是承诺。

⚠️ **本书尚未规定我们自己的组件用哪个开源许可证**（见第 13 部分待决）。这个决定要在**第一个仓库 push 之前**做完——事后改许可证需要所有贡献者同意。
...
| 102 | **参考实现纪律**：每个组件先看现实开源 ERP 的对应模块，但**读设计永远安全、复制代码只限 Apache-2.0/MIT/BSD、严禁复制或翻译 GPL/LGPL/AGPL 代码**（§3.2.1） | 凭空设计的领域模型漏掉的边界情形，要到客户上线三个月后才暴露；而抄了 copyleft 代码，`customer_fork` 的「闭源且归客户所有」在法律上当场失效 | ① 自己发明一套订单状态机，半年后发现漏了「部分发货 + 部分退货」；② 把 Odoo 的算法「用 Go 重写一遍」，以为不算衍生作品 |
