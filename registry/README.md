...
| `permissions.tsv` | 权限键册（`key`/`title`/`type`/`owner_component`/`deprecated`） | 改一个已发布的键名，所有已分配它的角色**静默失权且不报错**（设计书第 14 章 §14.1.2）。内容来自各组件 `assembly.yaml` 的 `permissions` 段，`be-ops permissions --root .` 聚合产出（阶段三 Task 3 转正，`internal/authzreg` 判据） |
...
3b. `_shell-` 前缀的行（阶段四附加 Task 0.4 新增，`servedBy` 落地后）是**外壳自己**——跟 `_infra-` 的豁免理由不一样：`_infra-` 是"根本不是 brickKit 组件"，`_shell-` 是"是真实 brickKit 组件（有自己的 `component.yaml`，真的会被 `brickkit up` 起一个容器），但不属于设计书 62 个业务组件的固定名录"，同样不计入 62 那个数。跟真实组件/带外容器撞端口一样会被判定成真冲突，不享有 `_infra-` 那种"互相之间允许同端口"的豁免（外壳之间、外壳与任何东西都不该共用端口）。
...
6. `permissions.tsv` 废弃走 `deprecated` 墓碑列，**键名永不回收**——`be-ops permissions` 只增量合并，孤儿 key（这次扫描没声明、也没标 `deprecated`）会打警告但绝不自动删行，废弃靠人手工在表里打墓碑。
