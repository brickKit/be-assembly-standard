...
| 11 | 照着某个开源 ERP 的模块结构直接搬一遍 | **没有立刻的症状**，但会把它的通病一起搬进来——最典型的是「所有模块同进程同库、可跨表 JOIN」，而那正是本项目存在的理由（设计书 §1.1）。它们同进程同库，我们独立进程独立 schema，**结构不可能相同** | 总纲 §4 SOP-R |

⚠️ **第 11 条的正确做法是 SOP-R 的三步法**：① 先自己按四把尺子设计一版 → ② 理不清的地方才去看它们怎么实现 → ③ 回来自己想清楚再写。**带着自己的方案去读别人的，才看得出差别在哪；空着脑袋去读只会照搬。** 许可证方面不必紧张：借鉴逻辑不构成衍生作品，要避免的只有「打开对方源文件逐行照抄」。
