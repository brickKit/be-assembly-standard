...
| 已建组件 | **6 个建完**：`mdm-customer@1.0.2`、`mdm-product@1.0.2`（只读枢纽，九门禁全绿）、`erp-inventory@1.0.4`（物理命令枢纽，TCC 三件套 + claim-first 幂等 + 条件更新防超卖）、`erp-finance@1.0.2`（事件汇枢纽，`FinanceService` 11 rpc，消费三个不同来源的事件，幂等过账两层防护）、`erp-sales@1.0.4`（本阶段唯一的链上一环，四条强依赖边全部真实 gRPC 调用，`ConfirmOrder` 的 TCC 补偿链）、`infra-authz@1.0.1`（阶段三 Task 4，权限体系里唯一持久化状态的组件：`GET /authz/bundle` 策略下发、角色模型纯并集无 Deny、`stale_since` 有界列表、自举机制、管理 API 自吃自己下发的 bundle；24 条真机测试全绿）。六者 REST 路由都已用 `besdk.GET/POST/PATCH(perm)` 注册但全部暂标 `Public`，等 Task 5/6 换真键。⚠️ 前五个组件的版本号本次（Task 4）又集体往上跳了一格——真机 `brickkit up` 核对时发现它们的 `deployment.image` 一直落后 `metadata.version`（镜像从未在版本升级后重新构建过，容器名对、代码是旧的），详细经过见踩坑记录类别 C（C5）与 [`03-阶段三`](docs/plans/03-阶段三-业务闭环.md) Task 4 小节；`check-version` 门禁已加一条判据防止同类问题再犯 |
| 工具仓库 | `be-sdk-go@v0.1.9`、`be-ops@v0.1.4`、`be-acceptance@v0.3.1`（`SystemClient`/裸路由扫描扩展到 Python/TS + tier0 容器名修复）、`be-sdk-python@v0.1.0`、`be-sdk-ts@v0.1.0`——五个全部已 tag（带注解）并被装配仓库的 submodule 指针跟踪。⚠️ Task 4 发现并修过一类"git 本身行为反直觉"的坑（轻量 tag 让 `git submodule status` 显示假漂移），详见踩坑记录类别 F |
