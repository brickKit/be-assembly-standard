...
| 项 | 值 |
|---|---|
| 阶段 | **阶段一已出档** ✅。**阶段二已出档** ✅（[`02-阶段二`](docs/plans/02-阶段二-验平台.md)：Task 1–21 + 出档 Part 3 四件事全部完成，复盘见 [`02-阶段二复盘`](docs/retrospectives/02-阶段二-验平台-复盘.md)）。**阶段三 · 业务闭环进行中**（[`03-阶段三`](docs/plans/03-阶段三-业务闭环.md)，每个 Task 的详细完成情况见该计划文档本身，这里只留最新状态）：9 个新组件 + 两个新 SDK，核心是把阶段二 5 个组件的权限判定从 fail-closed stub 换成 `infra-authz` 真实 bundle。九份组件设计计划全部齐备（见 [`docs/design/README.md`](docs/design/README.md) 索引）。**Task 1–6 已完成**：`be-sdk-python`/`be-sdk-ts` 骨架建仓库接入军火库；`make gates` 的两条扫描 + `registry/permissions.tsv`/`data-scopes.tsv` 转正 + 新增 `make version-check`；`infra-authz` 建仓库实现完成（见上方「已建组件」行）；三个 `be-sdk-*` 的 `RequirePermission`/`ScopeOf` 从 fail-closed stub 换成真实判定（Task 5，详见 [`03-阶段三`](docs/plans/03-阶段三-业务闭环.md) Task 5 小节）；阶段二五个组件 + `infra-authz` 自身的权限键从 `besdk.Public` 换成真实键，`erp-sales`（org/owner）/`erp-inventory`（warehouse）/`erp-finance`（legal_entity）的 `data_scopes` 真正生效（Task 6，详见该计划文档 Task 6 小节，含"仓库/法人访问分配数据归各组件自己维护"这条讨论定案）。⚠️ 全系统目前仍是 fail-closed：`infra-iam-casdoor`（真实 JWT 签发方）要到 Task 7 才建仓库，在此之前没有任何调用方能拿到验签通过的 JWT。**下一步：Task 7（`infra-iam-casdoor` 建仓库，全系统第一次有真实 JWT 可用）** |
| 已建组件 | **6 个建完**：`mdm-customer@1.0.3`、`mdm-product@1.0.3`（只读枢纽，九门禁全绿，`data_scopes: none`）、`erp-inventory@1.0.5`（物理命令枢纽，TCC 三件套 + claim-first 幂等 + 条件更新防超卖 + `warehouse` 维数据权限：新增 `warehouse_access` 分配表 + 管理接口）、`erp-finance@1.0.3`（事件汇枢纽，`FinanceService` 11 rpc + `legal_entity` 维数据权限：新增 `legal_entity_access` 分配表 + 管理接口）、`erp-sales@1.0.5`（本阶段唯一的链上一环，四条强依赖边全部真实 gRPC 调用，`ConfirmOrder` 的 TCC 补偿链 + `org`/`owner` 两维数据权限：`?view=mine\|dept` 两个视图）、`infra-authz@1.0.2`（权限体系里唯一持久化状态的组件：`GET /authz/bundle` 策略下发、角色模型纯并集无 Deny、`stale_since` 有界列表、自举机制、管理 API 自吃自己下发的 bundle）。六者 REST 路由已全部换成真实权限键（Task 6），`data_scopes` 三个组件真正生效——但 `iamJwksUrl` 全部留空（`infra-iam-casdoor` 还没建），当前仍是 fail-closed，见 [`03-阶段三`](docs/plans/03-阶段三-业务闭环.md) Task 6 小节 |
| 工具仓库 | `be-sdk-go@v0.2.1`、`be-ops@v0.1.4`、`be-acceptance@v0.3.2`、`be-sdk-python@v0.2.0`、`be-sdk-ts@v0.2.0`——五个全部已 tag（带注解）并被装配仓库的 submodule 指针跟踪。`be-sdk-go@v0.2.1` 新增 `ContextWithClaims`（供业务组件单元测试构造带 Claims 的 ctx，Task 6 写 erp-inventory 测试时发现的缺口）。⚠️ Task 4 发现并修过一类"git 本身行为反直觉"的坑（轻量 tag 让 `git submodule status` 显示假漂移），详见踩坑记录类别 F |
