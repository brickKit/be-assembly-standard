...
| 项 | 值 |
|---|---|
| 阶段 | **阶段一已出档** ✅。**阶段二 · 验平台进行中**（见 [`02-阶段二`](docs/plans/02-阶段二-验平台.md)）：Task 1–9 已完成（还账三笔 + `mdm-product` 全部建成 + `erp-inventory` 建仓库/CP-4 + 契约 + 迁移），Task 10 起未开工 |
| 已建组件 | **2 个建完**：`mdm-customer@1.0.0`、`mdm-product@1.0.0`（都是只读枢纽，九门禁全绿，六条 REST 路由已用 `besdk.GET/POST/PATCH(perm)` 注册，全部标 `Public` 待阶段三换真键）。`mdm-product` 验证了模板可复用（`partition` 包字面零改动，唯一真正新写的是 `ConvertQuantity`）。**`erp-inventory` 契约+迁移已定**（`InventoryService` 9 rpc；`inventory_movements` 月分区、`inventory_balances` 不分区+双重防超卖 CHECK，真机验证过条件更新/唯一约束/CHECK 全部生效），实现（并发防超卖测试是核心）留给 Task 10。阶段二再加 2 个未开工：`erp-finance`、`erp-sales`（四份设计计划已写完，见 `docs/design/`） |
| 工具仓库 | `be-sdk-go@v0.1.7`、`be-ops@v0.1.2`、`be-acceptance@v0.1.2`（三个都已 tag 并被装配仓库的 submodule 指针跟踪） |
