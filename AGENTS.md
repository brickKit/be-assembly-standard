...
| 项 | 值 |
|---|---|
| 阶段 | **阶段一已出档** ✅。**阶段二已出档** ✅（[`02-阶段二`](docs/plans/02-阶段二-验平台.md)：Task 1–21 + 出档 Part 3 四件事全部完成，复盘见 [`02-阶段二复盘`](docs/retrospectives/02-阶段二-验平台-复盘.md)）。**阶段三 · 业务闭环已写计划、尚未开工**（[`03-阶段三`](docs/plans/03-阶段三-业务闭环.md)）：9 个新组件（`infra-authz`/`infra-iam-casdoor`/`infra-workflow`/`infra-notification`/`integration-im-dingtalk`/`infra-print`/`infra-bff-mobile`/`frontend-standard`/`crm-opportunity`）+ 两个新 SDK（`be-sdk-python`/`be-sdk-ts`），核心是把阶段二 5 个组件的权限判定从 fail-closed stub 换成 `infra-authz` 真实 bundle。写计划前核对 §9.6.1/§14.3/附录 H 发现设计书原漏了 `infra-authz`（已订正，阶段三是 9 个组件不是 8 个，凑成 14 个不是 13 个）。**九份组件设计计划已全部齐备**（`infra-authz` 在阶段二随第 14 章写好，其余 8 份已补齐，见 [`docs/design/README.md`](docs/design/README.md) 索引）。⚠️ 写这批计划时又回写了设计书四处：§6.1 两句被第 14 章推翻的话、**附录 D 的登录时序图**（旧图是决策 115 之前的形态）、§5.11.1 的「审批路由」判定。**Task 1/2/3 均已完成**：`be-sdk-python`（`Config`/`Runtime`/`Module`/`run_standalone`/`new_fastapi_app`/`endpoint`/`with_tx` 真实实现，19 条测试全绿；`asyncpg.Pool` 在 `release()` 时自动 `RESET ALL` 的发现记入踩坑记录 A6）与 `be-sdk-ts`（`Config`/`Runtime`/`Module`/`runStandalone`/`newGraphQLServer`（Yoga + graphql-armor 深度限制 + Persisted Operations）真实实现，18 条测试全绿；Yoga 默认 `maskedErrors` 会掩盖权限错误与深度校验错误、`prom-client` 已被官方标记 deprecated 换成 `@prometheus-io/client`，记入踩坑记录 A7）都已建仓库接入军火库。两者的 `otel`/`logging`/`metrics` 等横切能力仍是签名骨架，留到真正需要它们的组件（Task 9/10/11）之前用 TDD 填平；`authz`/`scope`/`client` 按 be-sdk-go 当前 fail-closed stub 形状写，Task 5 三个 SDK 一起换真实判定。`make gates` 的 `SystemClient` 误用扫描 + 裸路由扫描（`bare-gin-scan` 改名 `bare-route-scan`）已扩展到 Python/TS（`be-acceptance@v0.3.0`），新拍板并写回总纲 SOP-B 两个此前没文档化的目录约定：Python 组件 `backend/app/http`/`backend/app/grpc`、TS GraphQL BFF 的 resolver 一律放 `src/resolvers/`（`infra-bff-mobile.md` 待决问题 #1 就此了结）；`registry/permissions.tsv`/`data-scopes.tsv` 已从空表转正为真实产出（`be-ops@v0.1.4` 新增 `permissions`/`data-scopes` 两个子命令，`internal/authzreg` 包，已用阶段二五个真实组件跑出 21 条权限键 + 4 条数据权限声明）；新增 `make version-check` 扫全部 submodule 的 tag 漂移，**第一次真跑就抓到 `be-sdk-python`/`be-sdk-ts` 建仓库后忘了打 `v0.1.0` 的真实漂移**，当场补上。**下一步：第 1 部分 Task 4（`infra-authz` 建仓库 → CP-9 → 契约 + 实现，权限判定真正上线三件套的第一件）** |
| 已建组件 | **5 个建完**：`mdm-customer@1.0.1`、`mdm-product@1.0.1`（只读枢纽，九门禁全绿）、`erp-inventory@1.0.3`（物理命令枢纽，TCC 三件套 + claim-first 幂等 + 条件更新防超卖，`besdk.Consume` 第一个真实调用方；v1.0.1/v1.0.2 补了 `GetReservationStatus` 按 `idempotency_key` 查询的路径——`erp-sales` 实现时发现的契约缺口，纯新增字段不破坏兼容）、`erp-finance@1.0.1`（事件汇枢纽，九门禁全绿，`FinanceService` 11 rpc；消费三个不同来源的事件；幂等过账两层防护、期间权威判定与 `post_no` 生成搭同一把锁、红字冲销；v1.0.1 对齐 `sales.order.created.v1` 的真实字段名）、**`erp-sales@1.0.2`（本阶段唯一的链上一环，四条强依赖边全部真实 gRPC 调用；`ConfirmOrder` 的 TCC 补偿链——`contracts/vendor/` 只读镜像 + 本地生成客户端 stub 是本项目第一次跨组件调用立的先例，记在设计计划 §9；真故障注入测试打真实网络请求到真实运行的三个依赖容器，覆盖真实库存不足/真实超时状态内省/真实补偿三条路径）**。五者的 REST 路由都已用 `besdk.GET/POST/PATCH(perm)` 注册，全部标 `Public` 待阶段三换真键。⚠️ `mdm-customer`/`mdm-product`/`erp-inventory` 三者的 `1.0.1`/`1.0.1`/`1.0.3` 是本次补的版本号——Task 19 修 `be-sdk-go` 时顺手升了它们的 `go.mod`（`erp-inventory` 还多了权限路由迁移那笔），但当时没跟着打新 tag，`component.yaml` 的 `version` 字段停留在旧值，直到这次核对 `git submodule status` 才发现三个仓库 HEAD 已经领先最新 tag（详见 `git log <上一个tag>..HEAD`）；`erp-sales` 的 `dependencies.components` 精确版本号跟着同步到 `1.0.2`，否则 `brickkit up` 会把同一个组件解析成两个版本节点 |
| 工具仓库 | `be-sdk-go@v0.1.9`（`erp-sales` 实现时发现并修复：`Config` 从 v0.1.0 起就没有把 camelCase 配置键转成平台真实注入的 SCREAMING_SNAKE_CASE，四个已发布组件因默认值恰好等于真实值而未暴露）、`be-ops@v0.1.4`（新增 `permissions`/`data-scopes` 子命令）、`be-acceptance@v0.3.0`（`SystemClient`/裸路由扫描扩展到 Python/TS）、`be-sdk-python@v0.1.0`、`be-sdk-ts@v0.1.0`（这两个建仓库时忘了打 tag，`make version-check` 第一次真跑就抓到，Task 3 补上）——五个全部已 tag 并被装配仓库的 submodule 指针跟踪 |
