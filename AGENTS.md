...
| 已建组件 | **14 个建完**：`mdm-customer@1.0.3`、`mdm-product@1.0.3`（只读枢纽，九门禁全绿，`data_scopes: none`）、`erp-inventory@1.0.6`（物理命令枢纽，TCC 三件套 + claim-first 幂等 + 条件更新防超卖 + `warehouse` 维数据权限）、`erp-finance@1.0.3`（事件汇枢纽，`FinanceService` 11 rpc + `legal_entity` 维数据权限）、`erp-sales@1.0.8`（本阶段唯一的链上一环，四条强依赖边全部真实 gRPC 调用，`ConfirmOrder` 的 TCC 补偿链 + `org`/`owner` 两维数据权限，补偿连续失败 3 次真接 `infra-workflow` 建异常待办并消费完成事件恢复订单）、`infra-authz@1.0.2`（权限体系里唯一持久化状态的组件：`GET /authz/bundle` 策略下发、角色模型纯并集无 Deny、`stale_since` 有界列表、自举机制）、`infra-iam-casdoor@1.0.2`（`slot:iam` Default 成员：两个 token 架构、refresh token rotation+重放检测、Casdoor webhook 桥接、首次初始化每次启动对账）、`infra-workflow@1.0.1`（零强依赖零弱依赖的审批待办中心：`CreateTask`/`CloseTask`/`CancelTask` 三命令统一走 `command_idempotency` 表 claim-first，四个状态流转动作统一 `ErrNotPending` 策略，`owner`/`org` 两维数据权限——`GET /tasks` 用 OR、`GET /admin/tasks` 用 toggle，实现中修正过一次真实的 fail-open 数据权限漏洞并补了回归测试；v1.0.1：补 `event_outbox` 周分区维护循环，建仓库时漏掉的真实缺口，第 5 周起写入本会直接失败，见踩坑记录 C12）、**`infra-notification@1.0.2`（新，路由中枢：零依赖零出边完全活在事件图上，两层通道偏好 + critical 类别不可关闭，通知状态机 `PENDING→ACCEPTED→SENT/RETRYING→FAILED_PERMANENT`，实现中修正过一次真实的事件 version 单调性 bug——重试重发若不递增 version 会被下游 `event_inbox` 静默吞掉）**、**`integration-im-dingtalk@1.0.2`（新，`channel:im` 族第一个成员：钉钉 access_token 缓存刷新、手机号→userid 缓存 + 过期清理、异步两段式投递状态机含延迟回查重试；用真实钉钉个人团队凭据真机验证过完整投递链路，验证中修正三个真实 bug——"强制刷新有时候不刷新"、"应用未开通权限"被误判可重试、`GetSendResult` 假设的响应字段真实不存在导致永远卡在 `ACCEPTED`，见上方阶段行）**、**`infra-print@1.0.2`（新，全系统第一个 Python 组件：纯函数打印渲染中心，零依赖零出边，`Render`/`BatchGetTemplates`/`ListTemplates` 三个 gRPC rpc，PDF（WeasyPrint + Jinja2）/ZPL（纯文本替换）双渲染路径，`Render` 是全系统唯一不带幂等键的写命令 rpc；真机验证过真实渲染出带中文的 A4 送货单 PDF，实现中意外发现并修复了 `be-sdk-python` 一个影响全部 Python 组件的 bug——`/healthz` 对 HEAD 请求返回 405，见上方阶段行）**、**`infra-bff-mobile@1.0.2`（新，全系统第一个 TypeScript 组件：GraphQL BFF，手写 schema 按前端页面裁剪，零强依赖零数据库，五条弱依赖全部可选；`data_scopes: none` 的客户/产品走 gRPC `BatchGet`+DataLoader，有真实数据权限的订单/库存/待办改走 REST 转发——全项目 gRPC 组件间协议不做数据权限过滤，这是实现时发现的真实缺口，见上方阶段行；v1.0.2：弱依赖版本号同步跟进 erp/sales@1.0.8、infra/workflow@1.0.1）**、**`frontend-standard@1.0.0`（新，PC 独立 Vite SPA + 移动端 Uni-app H5，唯一没有后端形态的组件：`data_scopes: none`、零依赖零数据库、`component.yaml` 的 `dependencies.components` 永远空数组——前端只认网关；真机验证过完整登录链路与"前端判定不是安全边界"这条设计原则，见上方阶段行）**、**`crm-opportunity@1.0.1`（新，阶段三第一个业务组件、CRM 域第一个组件：商机全生命周期，两条强依赖 `mdm/customer`/`mdm/product` 均只读 `BatchGet`、无 TCC 编排包，明确不依赖任何 `erp-*`/`infra-workflow`——§1.4 铁律"CRM 与 ERP 零同步边，仅事件握手"第一次真正落地；`MarkWon` 是链路的扳机，真机验证过状态转 WON + Outbox 记录 + 事件真的发布到 NATS 总线，见上方阶段行）**。十三个后端组件的 `iamJwksUrl`/`authzBundleUrl` 现在都指向真实运行的对端，见 [`03-阶段三`](docs/plans/03-阶段三-业务闭环.md) Task 7/8/9/10/11/13 小节 |
