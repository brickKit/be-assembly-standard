...
| 项 | 值 |
|---|---|
| 阶段 | **阶段一已出档** ✅。**阶段二已出档** ✅（[`02-阶段二`](docs/plans/02-阶段二-验平台.md)：Task 1–21 + 出档 Part 3 四件事全部完成，复盘见 [`02-阶段二复盘`](docs/retrospectives/02-阶段二-验平台-复盘.md)）。**阶段三 · 业务闭环进行中**（[`03-阶段三`](docs/plans/03-阶段三-业务闭环.md)，每个 Task 的详细完成情况见该计划文档本身，这里只留最新状态）：9 个新组件 + 两个新 SDK，核心是把阶段二 5 个组件的权限判定从 fail-closed stub 换成 `infra-authz` 真实 bundle。九份组件设计计划全部齐备（见 [`docs/design/README.md`](docs/design/README.md) 索引）。**Task 1–5 已完成**：`be-sdk-python`/`be-sdk-ts` 骨架建仓库接入军火库；`make gates` 的两条扫描 + `registry/permissions.tsv`/`data-scopes.tsv` 转正 + 新增 `make version-check`（`be-ops@v0.1.4`/`be-acceptance@v0.3.0`）；`infra-authz@1.0.1` 建仓库实现完成（见上方「已建组件」行）；三个 `be-sdk-*` 的 `RequirePermission`/`ScopeOf` 从 fail-closed stub 换成真实判定——JWT 本地验签 + 15 秒 bundle 轮询 + 纯并集权限键查找 + 新增 `Authenticated` 哨兵值，判定链三份 SDK 逐字对应，详见 [`03-阶段三`](docs/plans/03-阶段三-业务闭环.md) Task 5 小节，三者均已 `v0.2.0` 打 tag。**下一步：Task 6（阶段二五个组件 + `infra-authz` 自身回填真实权限键，`data_scopes` 真正启用）** |
| 已建组件 | **6 个建完**：`mdm-customer@1.0.2`、`mdm-product@1.0.2`（只读枢纽，九门禁全绿）、`erp-inventory@1.0.4`（物理命令枢纽，TCC 三件套 + claim-first 幂等 + 条件更新防超卖）、`erp-finance@1.0.2`（事件汇枢纽，`FinanceService` 11 rpc，消费三个不同来源的事件，幂等过账两层防护）、`erp-sales@1.0.4`（本阶段唯一的链上一环，四条强依赖边全部真实 gRPC 调用，`ConfirmOrder` 的 TCC 补偿链）、`infra-authz@1.0.1`（阶段三 Task 4，权限体系里唯一持久化状态的组件：`GET /authz/bundle` 策略下发、角色模型纯并集无 Deny、`stale_since` 有界列表、自举机制、管理 API 自吃自己下发的 bundle；24 条真机测试全绿）。六者 REST 路由都已用 `besdk.GET/POST/PATCH(perm)` 注册但全部暂标 `Public`，等 Task 6 换真键（判定本体已在 Task 5 换成真实实现，只差调用点的权限键字面量）。⚠️ 前五个组件的版本号本次（Task 4）又集体往上跳了一格——真机 `brickkit up` 核对时发现它们的 `deployment.image` 一直落后 `metadata.version`（镜像从未在版本升级后重新构建过，容器名对、代码是旧的），详细经过见踩坑记录类别 C（C5）与 [`03-阶段三`](docs/plans/03-阶段三-业务闭环.md) Task 4 小节；`check-version` 门禁已加一条判据防止同类问题再犯 |
| 工具仓库 | `be-sdk-go@v0.2.0`、`be-ops@v0.1.4`、`be-acceptance@v0.3.1`（`SystemClient`/裸路由扫描扩展到 Python/TS + tier0 容器名修复）、`be-sdk-python@v0.2.0`、`be-sdk-ts@v0.2.0`——五个全部已 tag（带注解）并被装配仓库的 submodule 指针跟踪。三个 `be-sdk-*` 的 `v0.2.0` 是 Task 5 的权限判定真实实现。⚠️ Task 4 发现并修过一类"git 本身行为反直觉"的坑（轻量 tag 让 `git submodule status` 显示假漂移），详见踩坑记录类别 F |
