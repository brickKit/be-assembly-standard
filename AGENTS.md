...
| 某个具体组件 | `docs/design/<仓库名>.md`（为什么这样设计）+ `components/<scope>/<name>/AGENTS.md`（怎么改它） |
...
前六条的共同点：**代码看起来完全正确，测试也能过，症状要么极难联想，要么根本没有症状。**
...
3. **我改的这处，四份文档里哪几份该跟着改？** 边界/契约/事件变了 → 先改 `docs/design/<仓库名>.md`；功能变了 → README；用法或结构变了 → `docs/手册.md`；禁令或判据变了 → 该仓库的 `AGENTS.md`。**顺序是文档先行，不是代码先行。**
