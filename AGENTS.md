...
| 项 | 值 |
|---|---|
| 阶段 | **阶段一已出档** ✅。**阶段二 · 验平台进行中**（见 [`02-阶段二`](docs/plans/02-阶段二-验平台.md)）：Task 1–8 已完成（还账三笔 + `mdm-product` 全部建成 + `erp-inventory` 建仓库/CP-4 + 契约），Task 9 起未开工 |
| 已建组件 | **2 个建完**：`mdm-customer@1.0.0`、`mdm-product@1.0.0`（都是只读枢纽，九门禁全绿，六条 REST 路由已用 `besdk.GET/POST/PATCH(perm)` 注册，全部标 `Public` 待阶段三换真键）。`mdm-product` 验证了模板可复用（`partition` 包字面零改动，唯一真正新写的是 `ConvertQuantity`）。**`erp-inventory` 契约已定**（`InventoryService` 9 rpc，TCC 四件套不进 REST，`buf lint` 绿），迁移/实现留给 Task 9–10。阶段二再加 2 个未开工：`erp-finance`、`erp-sales`（四份设计计划已写完，见 `docs/design/`） |
