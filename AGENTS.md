...
| 项 | 值 |
|---|---|
| 阶段 | **阶段一已出档** ✅。**阶段二已出档** ✅（[`02-阶段二`](docs/plans/02-阶段二-验平台.md)：Task 1–21 + 出档 Part 3 四件事全部完成，复盘见 [`02-阶段二复盘`](docs/retrospectives/02-阶段二-验平台-复盘.md)）。**阶段三 · 业务闭环进行中**（[`03-阶段三`](docs/plans/03-阶段三-业务闭环.md)，每个 Task 的详细完成情况见该计划文档本身，这里只留最新状态）：9 个新组件 + 两个新 SDK，核心是把阶段二 5 个组件的权限判定从 fail-closed stub 换成 `infra-authz` 真实 bundle。九份组件设计计划全部齐备（见 [`docs/design/README.md`](docs/design/README.md) 索引）。**Task 1–4 已完成**：`be-sdk-python`/`be-sdk-ts` 骨架建仓库接入军火库（横切能力签名骨架留给 Task 9/10/11 前 TDD 填平）；`make gates` 的两条扫描 + `registry/permissions.tsv`/`data-scopes.tsv` 转正 + 新增 `make version-check`（`be-ops@v0.1.4`/`be-acceptance@v0.3.0`）；`infra-authz@1.0.0` 建仓库实现完成——见上方「已建组件」行。**下一步：Task 5（三个 `be-sdk-*` 补真实权限判定：`RequirePermission` 查内存 bundle map、15 秒轮询循环、`ScopeFilter` 真的下场，外加 Task 4 发现的 `besdk.Authenticated` 哨兵值缺口）** |
| 已建组件 | **6 个建完**：`mdm-customer@1.0.1`、`mdm-product@1.0.1`（只读枢纽，九门禁全绿）、`erp-inventory@1.0.3`（物理命令枢纽，TCC 三件套 + claim-first 幂等 + 条件更新防超卖，`besdk.Consume` 第一个真实调用方；v1.0.1/v1.0.2 补了 `GetReservationStatus` 按 `idempotency_key` 查询的路径——`erp-sales` 实现时发现的契约缺口，纯新增字段不破坏兼容）、`erp-finance@1.0.1`（事件汇枢纽，九门禁全绿，`FinanceService` 11 rpc；消费三个不同来源的事件；幂等过账两层防护、期间权威判定与 `post_no` 生成搭同一把锁、红字冲销；v1.0.1 对齐 `sales.order.created.v1` 的真实字段名）、`erp-sales@1.0.2`（本阶段唯一的链上一环，四条强依赖边全部真实 gRPC 调用；`ConfirmOrder` 的 TCC 补偿链——`contracts/vendor/` 只读镜像 + 本地生成客户端 stub 是本项目第一次跨组件调用立的先例，记在设计计划 §9；真故障注入测试打真实网络请求到真实运行的三个依赖容器，覆盖真实库存不足/真实超时状态内省/真实补偿三条路径）、**`infra-authz@1.0.0`（阶段三 Task 4，权限体系里唯一持久化状态的组件：`GET /authz/bundle` 策略下发`ETag`/304、角色模型纯并集无 Deny、`stale_since` 有界列表是 `role_changes` 与 `user_roles.expires_at` 两个来源的 UNION、自举机制解决"第一个管理员怎么来"、管理 API 自吃自己下发的 bundle；24 条真机测试全绿，真机端到端冒烟覆盖 REST 全链路 + gRPC，过程中发现并修复 pgx 不自动解析 PostgreSQL `text[]` 的真实 bug）**。前五者的 REST 路由都已用 `besdk.GET/POST/PATCH(perm)` 注册，全部标 `Public` 待阶段三换真键；`infra-authz` 自己的路由同样暂标 `Public`（真实权限键 `infra.authz.admin` 已在 `assembly.yaml` 声明），一起等 Task 5/6。⚠️ `mdm-customer`/`mdm-product`/`erp-inventory` 三者的 `1.0.1`/`1.0.1`/`1.0.3` 是本次补的版本号——Task 19 修 `be-sdk-go` 时顺手升了它们的 `go.mod`（`erp-inventory` 还多了权限路由迁移那笔），但当时没跟着打新 tag，`component.yaml` 的 `version` 字段停留在旧值，直到这次核对 `git submodule status` 才发现三个仓库 HEAD 已经领先最新 tag（详见 `git log <上一个tag>..HEAD`）；`erp-sales` 的 `dependencies.components` 精确版本号跟着同步到 `1.0.2`，否则 `brickkit up` 会把同一个组件解析成两个版本节点 |
| 工具仓库 | `be-sdk-go@v0.1.9`（`erp-sales` 实现时发现并修复：`Config` 从 v0.1.0 起就没有把 camelCase 配置键转成平台真实注入的 SCREAMING_SNAKE_CASE，四个已发布组件因默认值恰好等于真实值而未暴露）、`be-ops@v0.1.4`（新增 `permissions`/`data-scopes` 子命令）、`be-acceptance@v0.3.0`（`SystemClient`/裸路由扫描扩展到 Python/TS）、`be-sdk-python@v0.1.0`、`be-sdk-ts@v0.1.0`（这两个建仓库时忘了打 tag，`make version-check` 第一次真跑就抓到，Task 3 补上）——五个全部已 tag 并被装配仓库的 submodule 指针跟踪。⚠️ Task 4 挂载 `infra-authz` 后核对 `git submodule status` 又发现一类新坑：`be-sdk-go`（`v0.1.8`/`v0.1.9`）与 `erp-finance`（`v1.0.0`/`v1.0.1`）当初把 tag 打成了轻量 tag（`git tag` 不带 `-a`），导致 `git submodule status`（内部用不带 `--tags` 的 `git describe`，只认带注解 tag）显示成假的版本漂移——已改打成带注解 tag 修复，`make version-check` 补一条判据（`git cat-file -t` 必须是 `tag`）防止同类问题再次不被发现，记入踩坑记录类别 F |
