...
| 工具仓库 | `be-sdk-go@v0.1.9`（`erp-sales` 实现时发现并修复：`Config` 从 v0.1.0 起就没有把 camelCase 配置键转成平台真实注入的 SCREAMING_SNAKE_CASE，四个已发布组件因默认值恰好等于真实值而未暴露）、`be-ops@v0.1.3`、`be-acceptance@v0.1.2`（三个都已 tag 并被装配仓库的 submodule 指针跟踪） |
