...
| 4 | ~~外壳合并配置：哪些组件进哪个外壳、端口分配、迁移执行顺序。~~ **已退休**——brickKit v0.4.2 新增原生 `BRICKKIT_SERVED_MEMBERS_CONFIG` 环境变量（数据来自注入引擎本来就已经给每个运行中组件算过一遍的结果），外壳启动器直接解析它，不再需要生成这一步 | 平台不提供合并部署支持（§13.6）——**写这一行时是真的，v0.4.2 之后不再成立** | **阶段四**，阶段四附加 Task 0.6 退休（`docs/plans/04b-验证记录.md`） |
...
| 7 | ~~每外壳的环境变量表：同外壳依赖 → `http://127.0.0.1:<端口>`；跨外壳/独立容器 → `http://<宿主机地址>:<端口>`~~ **已退休**——brickKit 原生的 `servedBy` 合并在生成阶段就直接把每一个 `*_ENDPOINT` 地址改写进外壳容器自己的环境变量 | 平台只往它自己生成的容器里注入，合并后那些容器不存在（§13.8）——**写这一行时是真的，`servedBy` 落地后不再成立** | **阶段四**，阶段四附加 Task 0.2/0.3 退休（`docs/plans/04b-验证记录.md`） |
| 8 | ~~shell-compose 的 `depends_on`：外壳之间的启动顺序~~ **已退休**——brickKit 原生的 `servedBy` 合并自己算外壳间启动顺序，它取代的那份手写 `infra/shell-compose.yml` 编排已删除 | 平台只排它生成的那些（§13.8）——**写这一行时是真的，`servedBy` 落地后不再成立** | **阶段四**，阶段四附加 Task 0.5 退休（`docs/plans/04b-验证记录.md`） |
