...
| 五、外壳合并部署 / K8s | 🔜 外壳：阶段四进行中，11/11 个 Go 模块已经真机验证过能合并进程运行，`brickkit.yaml` 的原子式真实切换还没做。K8s：**从未真机验证过**，见本节说明 |
| 六、升级与回滚 | 🔜 有第一个客户时 |
...
**外壳合并部署（Docker 单机，省资源）**：阶段四进行中——11/11 个 Go 组件已经在 `be-shell-go` 里真机验证过能合并进一个进程正常运行（含真实迁移、真实健康检查、真实权限中间件），但本仓库自己的 `brickkit.yaml` 从"14 个独立容器"切到"合并成几个外壳容器"这个原子式真实切换还没做。本节等切换完成后补齐具体命令，见 `docs/plans/04-阶段四-做外壳验拆回.md`。

⚠️ **外壳态与全拆态不能同时对着真实基础设施跑**——同一时刻只能有一个在跑（会撞端口、撞 NATS 广播、撞数据库状态），完整推理见《BrickEnterprise 设计书.md》§13.9。

**K8s 部署**：`brickkit` 平台本身支持 `deploy.target: k8s`（`brickkit up --context` 参数就是给这个用的），但**这条路径本仓库从来没有真机验证过**——设计书 §13.4 把它定位成"全拆态"专属的后续阶段（客户全面上云、购买 K8s 集群之后才会走到），且有一条硬约束：**`local: true`（外壳合并）只能配 `deploy.target: docker`，K8s 目标下 CLI 生成阶段直接报错**——上 K8s 意味着先把所有外壳拆回独立容器，没有"部分合并、部分上 K8s"这种中间态。本仓库当前连"合并进外壳"这一步都还没真机切换完成，更谈不上验证过 K8s 部署，因此本手册现在**不提供** K8s 部署教程——写一份没有真机跑过的教程，比没有教程更危险。真到了需要上 K8s 的阶段，应该先按设计书 §13.4 的路线图把 62 个组件全部独立部署，再对着真实 K8s 集群走一轮完整验证，之后再补这一节。
