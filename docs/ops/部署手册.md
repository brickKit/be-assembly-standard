...
```bash
# ① 创建共用网络（三份 compose 都接在它上面）—— 已有 make 目标，幂等
make net
...
# ④ 一键核对是否都对（容器/镜像/端口/健康/网络五项，已实现）
make check
```

> ⏳ **第 ③ 步会在阶段一 Task 3 完成后变成 `make up`**（两阶段：先整体预检，全过才启动）。现在第 ③ 步仍是原始 compose 命令，第 ④ 步的 `make check` 已经可用——`make up` 落地后本节会再更新一次。

`make check` 的输出**不是「全绿才算过」**，是「缺失可以接受、不对才报错」：还没启动的资源显示 `○ 缺失（make up 会拉起）`（黄色，不影响退出码），启动了但不对（镜像/端口/健康不符，或端口被别的容器/进程占了）才是 `✗`（红色，退出码非零）。第一次装机时你会看到全部 `○` 或混合状态，这是正常的。
