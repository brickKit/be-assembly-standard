...
| **开发者**（写组件的人） | 怎么写一个组件、测试写在哪一层、卡住了怎么办 | [`plans/00-总纲.md`](plans/00-总纲.md) 的 **§4 SOP-W** → 该组件的 [`design/<仓库名>.md`](design/)；「这个坑是不是已经踩过了」先查 [`dev/实测踩坑记录.md`](dev/实测踩坑记录.md)；日常怎么聚焦到几个组件、提交前要注意什么见 [`军火库维护手册.md`](军火库维护手册.md) |
