...
services:
  ...
  # ③ Traefik —— API 网关（默认）。Dashboard 从 8080 挪到 18080（总纲 §1.2）
  traefik:
    image: traefik:v3.0
...
entryPoints:
  ...
  websecure:
    address: ":443"
  traefik:
    address: ":18080"   # 总纲 §1.2：从 8080 挪开，让给外壳一的 mdm-customer
...
- 修正设计书附录 G 的四处端口撞车：Traefik Dashboard→18080、
  Keycloak→18081、Prometheus→19090、Kafka→19092（总纲 §1.2）
...
**读之前先复核两个仓库当前的 `LICENSE` 文件**，结果填进设计计划第 8 节的「许可证（已复核）」列——不要照抄总纲 §4 SOP-R 那张表，那是写作时的记录（开源项目改许可证不是罕见事）。

⚠️ **只读设计，不复制代码。** Odoo 是 LGPL-3；把它的代码复制进来（**「用 Go 重写一遍」同样是衍生作品**）会让客户 Fork 件无法闭源，而设计书 §3.2 明确 `customer_fork` 是「闭源且归客户所有」（总纲 §4 SOP-R 的 R-1c）。
...
|---|---|---|---|---|
| Odoo 17.0 | `addons/base` 的 `res.partner` | 一张 partner 表兼容客户/联系人的取舍 | LGPL-3 | 仅读设计 |
| Apache OFBiz 18.12 | `applications/party` 实体模型 | Party/Contact/PostalAddress 的关系 | Apache-2.0 | 仅读设计 |
