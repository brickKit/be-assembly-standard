...
##### ⚠️ 实测修正一：`${VAR}` 只能整值替换，不能嵌在字符串里

旧版把 DSN 写成 `"user=postgres password=${POSTGRES_PASSWORD} host=..."`，**跑不起来**。

beego 的 `ExpandValueEnv` 只在「整个值以 `${` 开头、以 `}` 结尾」时才展开；嵌在字符串中间的那一段是**字面量**。症状：

```
panic: pq: password authentication failed for user "postgres"
```

——而 **PostgreSQL 自己 healthy**，容器只是无限重启，看起来像 Casdoor 的毛病。

**正解**：整串 DSN 由 compose 拼好后作为一个变量注入（compose 那一层的插值没有这个限制），`app.conf` 里只写 `dataSourceName = ${CASDOOR_DSN}`。
> 已实测：改后 `be-casdoor` healthy、`RestartCount=0`、`GET /api/health` 返回 200、`casdoor` schema 里 44 张表。

##### ⚠️ 实测修正二：`brickkit_db` 与 `casdoor` schema 在 `make up` 时还不存在

`dataSourceName` 指向 `dbname=brickkit_db` + `search_path=casdoor`，而**这两样都要到 Task 8（`be-ops` 产出 2）才被创建**——排在 Task 3 的 `make up` 之后。少了它们，Casdoor 起不来，日志是 `no schema has been selected to create in`。

**正解**：给 postgres 挂一个 initdb 脚本，**只建带外容器需要的那一点点**：
...
##### ⚠️ 实测发现：Task 1 的 nats `command` 有一个从没被跑起来验证过的 bug

`docker compose config` 只检查语法，**从不启动容器**——所以 Task 1 的 nats 定义带着一个真的坏配置，一直到本任务第一次 `make up` 才暴露。

**症状**：`be-nats` 不停重启；日志是 nats-server 的用法说明（`flag provided but not defined: -max_payload`）。

**根因**：`--max_payload` **不是 nats-server 的 CLI 参数**——`nats-server --help` 里根本没有这一项，它只能写在配置文件里。原 compose 把它塞进了 `command: [..., "--max_payload", "8MB"]`，nats-server 一看到不认识的 flag 就打印用法说明并退出，`restart: unless-stopped` 让它不断重启循环。

**正解**：新增 `infra/nats/nats.conf`（只放 `max_payload: 8MB` 这一行），挂进容器、用 `-c` 指向它；`--jetstream` / `--store_dir` / `--http_port` 三个是合法 CLI flag，继续留在 `command` 里不用搬。

```yaml
# docker-compose.infra.yml 的 nats service，command 与 volumes 都要改
...
> 已实测：改后 `GET /varz` 里 `max_payload` 字段是 `8388608`（= 8MB），`be-nats` healthy。



##### ⚠️ 实测发现二：可观测性五件套里三个镜像没有 shell，`CMD-SHELL` 健康检查必然失败

本任务顺带用 `make obs-up` 完整跑了一遍 Task 1 交出的可观测性五件套——**同样从没被真正启动过**，只做过 `compose config` 语法检查。结果：`otel-collector`、`loki`、`tempo` 三个全部 `unhealthy`。

**根因是同一个**：这三个镜像里都**没有 `/bin/sh`**（`grafana/prometheus` 是完整系统镜像、有 shell，健康检查一次就过）。日志是：

```
exec: "/bin/sh": stat /bin/sh: no such file or directory
```

`otelcol-contrib` 二进制本身也没有内建的 healthcheck 子命令（`--help` 列出的子命令只有 `completion`/`components`/`featuregate`/`help`/`print-config`/`validate`）。**这几个镜像天生就做不到"容器内自检"**，这是十八条第 8 条（`FROM scratch`/distroless 基底）的同一类坑，只是这次踩在带外镜像上，不是我们自己的组件。

**正解**：这三个 service 直接**不定义 `healthcheck`**。Docker Compose 的 `--wait` 对没有 healthcheck 的服务只等到 `running` 状态就放行，不会卡在超时上。想确认它们真的活着，从宿主机侧探活（宿主机有 `curl`）：
...
> 已实测：三个都能拿到 200 / `{"status":"Server available",...}` / `ready`。

⚠️ **顺带一条不是 bug、但容易被误判成 bug 的现象**：**Loki 单体模式下，ingester 加入 ring 之后会再等约 15 秒才报 ready**（这是 Loki 自己文档记录的 `min_ready_duration` 行为，给 ring 状态收敛留缓冲）。`make obs-up` 刚返回时 `curl /ready` 可能拿到 `503` + `"Ingester not ready: waiting for 15s after being ready"`，**等一会儿再查就是 `ready`**——不需要重启、不需要改配置。

⚠️ **这条提醒了一件更通用的事**：**`docker compose config` 只验证 YAML 结构与变量插值，不代表容器真的能起来。** 凡是 `command:` 里手写的 CLI 参数，唯一可靠的验证是**真的启动一次**——这也是为什么本任务的步骤 4 把"跑 `make up`"列为必须步骤，而不是到此为止。可观测性五件套走的是 `make obs-up`，同一个道理，一并在本任务验证。
...
⚠️ **原计划这里写了 `docker stop my-postgres my-rustfs`，但那两个冲突在 Task 1 已经解决**（那两个容器早就停了）。跳过这一步，直接：

```bash
make up
```

⚠️ **已实测：第一次跑会被 Task 1 遗留的 nats 配置 bug 挡住**（见上方「实测发现」），报 `container be-nats is unhealthy`。这不是本任务引入的问题，是 Task 1 从没真正启动过 nats 才没暴露的。处理方式：
...
⚠️ **实测发现：`make rustfs-down` 这个目标不存在，计划原文假设错了。** 步骤 3 的 Makefile 只给 5 个**替换件**（minio/keycloak/kafka/rabbitmq/nginx）建了 `-down` 目标；5 个**默认资源**（postgres/nats/traefik/casdoor/rustfs）没有、也不该有单独的 `make <res>-down`——停一个不停别的在语义上没有意义，默认资源的开关粒度就是 `make up` / `make down`。

`optional.sh` 里那句"默认资源请走 make up/down"的守卫是给**脚本被绕过 Makefile 直接调用**这种情况兜底的防御性代码（比如未来某个自动化脚本手滑传错了资源名），本身是对的，只是不需要在 Makefile 里开一个入口去够到它。改成直接测脚本：
...
# 2. 端口两两不重复。唯一例外：slot:frontend 族共用 80（总纲 §2.1）
dups="$(awk -F'\t' '
  NF && $1 !~ /^#/ {
    if ($3 != "-" && $3 != "80") p[$3] = p[$3] " " $1
    if ($4 != "-")               p[$4] = p[$4] " " $1
...
4. **`1xxxx` 整段是平台保留，不许分配。** brickKit 给 `local: true` 组件及其依赖做宿主机映射时首选 `10000 + 容器端口`、fallback 从 `18080` 起递增（`internal/compose/local.go`）。组件端口最大 9221，所以平台实际会用到 **18080–19221**。带外容器一律走 `2xxxx`。
5. 唯一的端口复用例外是 `slot:frontend` 族共用 80（槽位互斥、永不共存、各自独立 Nginx 容器不进外壳）。校验脚本对此有专门放行。
```
...
- 62 个组件的 HTTP/gRPC 端口两两不重复，唯一例外是 slot:frontend 族共用 80
- 带外容器端口一并纳入（设计书的端口册只管 62 个组件，但外壳要把端口
