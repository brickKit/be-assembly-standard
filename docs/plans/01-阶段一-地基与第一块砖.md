...
| 7 | 接入 submodule + `be-sdk-go` 实现 SOP-L | 十项横切能力，`v0.1.0` |
...
| 15 | 失败测试（测试先于实现） | 红的测试 |
| 16 | 判设计模式（SOP-P）+ 实现、Dockerfile、7 个 CI 门禁 | HTTP 8080 + gRPC 9090 双端口 |
...
- Produces：`make gates` —— 跑全部门禁（本阶段只有 import-scan；拆回门禁在**阶段四**加）。
...
all: check-version test image migrate-idempotent dag-check contract-check import-scan docs-check
```
