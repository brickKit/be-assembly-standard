...
| 4 | 两张全局册子落盘 + 校验脚本 | `registry/ports.tsv`、`registry/schemas.tsv` |
| 5 | 军火库骨架 + `docs/design/` 脚手架 | `components/` 转 submodule、自洽闸门、设计计划模板 |
| 6 | 三个工具仓库骨架 → **检查点 CP-1** | `be-ops` / `be-acceptance` / `be-sdk-go` |
| 7 | 接入 submodule + `be-sdk-go` 实现 SOP-L | 十项横切能力 + `Runtime`/`Module`/`RunStandalone`/`NewGinEngine`，`v0.1.0` |
| 8 | `be-ops` 的三个先决产出 | 建库脚本 / `brickkit.yaml` 生成 / 端口册校验 |
...
| 13 | 两份 yaml | `component.yaml` + `assembly.yaml` |
...
⚠️ **这三个类型的签名一旦定下就是 61 个组件的骨架**，比横切函数更改不动——横切函数改签名是改调用点，`Runtime` / `Module` 改字段是改每个组件的 `module.New` 加外壳启动器。**定之前把设计书 §12.4 与 §12.5 读完。**

⚠️ **`DB` 是 `*sql.DB` 而不是 `*pgxpool.Pool`，驱动用 `pgx/v5/stdlib`。** 这一格是设计书 §12.4 想过的：外壳要把同一个池递给 8~21 个模块，那个类型就是模块入口契约的一部分；`database/sql` 是标准库类型、`sqlc` 直接支持、上面 `WithTx` / `PublishOutbox` 的签名本来就写在它上面。**不要用 `lib/pq`**（不再维护，且不支持我们要的一些 PG 类型）。
...
⚠️ `be-sdk-go` 已在设计书 §5.10 里从「`be-sdk-events-go`，reserve、只管事件」升为「必需件、SOP-L 十项横切能力」（决策 100）。创建仓库时描述里写清这一点。
...
- Produces：**`Runtime` / `Module` / `RunStandalone` / `NewGinEngine` 四个结构件**（Task 6 定的签名）。⚠️ 它们比十项横切能力更改不动——横切函数改签名是改调用点，`Runtime` 改字段是改**每个组件的 `module.New` 加外壳启动器**（§K、设计书 §12.5）。
...
| `metrics.go` | ① 每个 HTTP/gRPC 接口自动暴露 Rate/Errors/Duration；② 采集间隔 15s；③ Histogram bucket 数量有上限；④ **两个 `Runtime` 各自的 `Registry` 注册同名指标都成功**（守的是「进程内 21 个模块不能共用默认 registry」，§12.5.2） | §7.4、§12.5.2 |
...
git add -A && git commit -m "feat: SOP-L 十项横切能力 + Runtime/Module/RunStandalone/NewGinEngine 四个结构件"
...
func New(ctx context.Context, rt *besdk.Runtime) (*besdk.Module, error) {
	...
	return &besdk.Module{
		...
		// ⚠️ 必须接 ctx 并在 cancel 时返回——外壳要能关掉 21 个模块的全部后台循环。
		Start: func(ctx context.Context) error {
			if err := besdk.StartOutboxPump(ctx, rt.DB, schema, rt.NATS); err != nil {
				return err // ⚠️ 返回 error，不许 log.Fatal：一个模块退进程 = 整组 21 个组件一起没了
			}
...
module-check:  ## 铁律七：模块能被合进外壳（§12.5、§13.3 铁律七）
	...
	@bad="$$(grep -rnE 'os\.Getenv|os\.LookupEnv' backend/ --include='*.go' \
	 ...
	 if [ -n "$$bad" ]; then \
	   echo "✗ 模块代码里读了进程环境变量（21 个模块会互相顶掉，不报错）："; \
	   echo "$$bad"; exit 1; fi
...
2. **改设计书。** 复盘里发现设计书写的和实际不一样的，**先改设计书**（改不动就先在 `be-acceptance` 记一条红用例，再回 brickKit 仓库修）。绕过去的那一条，会在 61 个组件时变成 61 处绕法（§9.6.2）。
