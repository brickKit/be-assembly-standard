...
**出档条件：** `mdm-customer` 六项验收全绿（见 §2 开头那张表）+ 三份文档结构完整 + 全部门禁绿。
...
| 11 | 组件骨架（含 README 七节 + 手册骨架）→ **检查点 CP-2** | 目录树 + 两份组件文档 |
...
| 18 | 三份文档定稿 + `docs-check` 门禁 | `make docs-check` |
...
**Interfaces:**
- Produces：`make docs-check` —— 三份文档的机械检查（存在 / 章节齐全 / 无占位符）。**从这一刻起它进每个组件的 `make all`**，后面 57 个组件照抄。

⚠️ **这一步不是收尾的文书工作。** 它是本阶段唯一一次「照着刚做完的东西把三份文档校准」的机会——过了这个点，`mdm-customer` 会成为后面 57 个组件抄的模板，**文档抄错比代码抄错传播得更远**。
...
- [ ] **步骤 4：写 `infra/scripts/docs-check.sh`**

```bash
#!/usr/bin/env bash
# 三份文档的机械检查。内容质量靠 review，这里只查结构
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
...
for f in "$ROOT/docs/design/$REPO.md" "$DIR/README.md" "$DIR/docs/手册.md"; do
...
```makefile
docs-check:  ## 三份文档结构检查（总纲 §4 SOP-D）
...
```makefile
docs-check:  ## 检查某个组件的三份文档：make docs-check REPO=mdm-customer
...
git commit -m "docs: README 七节 + 手册六节定稿，接入 docs-check 门禁
...
**验证：** `make docs-check REPO=mdm-customer` 绿；组件仓库 `make all` 八项全绿。
...
make docs-check REPO=mdm-customer # ⑤ 三份文档结构完整
