...
| **第 2 部分 · 第一块砖** | | |
| 10 | 写 `mdm-customer` 的**设计计划** | `docs/design/mdm-customer.md` |
...
| 15 | 失败测试（测试先于实现） | 红的测试 |
| 16 | 实现、Dockerfile、7 个 CI 门禁 | HTTP 8080 + gRPC 9090 双端口 |
...
七节的标题必须一字不差（`make docs-check` 按标题名匹配）：`## 它能做什么` / `## 需要哪些基础资源` / `## 怎么起来` / `## 怎么用` / `## 配置项` / `## 借鉴了哪些项目` / `## 边界与禁令`。内容从 Task 10 的设计计划里搬，**不是重写一遍**。
...
`## 借鉴了哪些项目` 一节即使是「无」也要写明「无，纯自研」——**留空会让下一个人以为忘了写**。
...
sect "$DIR/README.md" '## 它能做什么' '## 需要哪些基础资源' '## 怎么起来'      '## 怎么用' '## 配置项' '## 借鉴了哪些项目' '## 边界与禁令'
