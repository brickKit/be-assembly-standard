...
- **完整度要求**：种子数据要把这个组件有意义的状态都各来一条，不是只种欢乐路径——比如一个有状态机的组件，种子数据里每个状态至少各出现一次；一个有失败分支的组件，至少种一条能触发失败分支的样本。这样打开项目就能直接点开每一种情况看，而不是只能看到「一切正常」那一种。**这条要求也是链式调用能成立的地基**：下游组件的 `seed` 要在上游产出的数据上组合出自己需要的东西（比如 `crm-opportunity` 需要"一个 `ACTIVE` 客户+一个 `ACTIVE` 产品"这种组合），上游的种子数据够丰富，下游才有得挑；上游只种一条，下游能拼出的花样也只有一种。
