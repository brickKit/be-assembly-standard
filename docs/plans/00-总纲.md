...
⚠️ **消费者测试自己构造/发布事件时，用测试私有的 subject，不要用真实生产 subject。** 同一台机器上 `brickkit up` 真实跑着的容器订阅的正是那个生产 subject，`besdk.Consume` 走的是 NATS 核心订阅（广播，不是竞争消费）——真实容器会跟本地测试的临时订阅者同时收到消息，谁先抢到 `event_inbox` 的认领谁的产出就留在数据库里，断言可能读到的是别人的结果（踩坑记录类别 E 的 E1/E2）。`besdk.Consume` 的 `fn` 不拿 `ev.Subject` 做业务判断，换成 `fmt.Sprintf("test.%s.%d", base, time.Now().UnixNano())` 这类测试私有名字完全安全，真实容器压根收不到。⚠️ 这条只适用于"测试自己发布事件去触发消费者"；验证"业务逻辑真的发到了正确的生产 subject"这件事本身的测试（如 `TestMarkWon_...`）必须用真实 subject。更根本的做法：这类干扰的前提是组装态的 14 个组件容器长期挂着，而那本不该是默认状态——见根 `AGENTS.md`"常用命令"一节。私有 subject 是"确实需要真实容器与测试同时在跑"时的防御性写法，不是替代关容器这件事。
...
**种子数据要同时服务的目标——这决定了"够不够丰富"怎么判断，不是拍脑袋定数量**

我们现在拥有的所有数据，从头到尾都是模拟数据，从来没有、也永远不会是真的——这件事本身就该被当成一种自由：既然没有"污染真实数据"的顾虑，种子数据的丰富度就应该往上限去够，而不是往"够用就行"去省。具体要同时撑起这几件事：

1. **完美测试所有逻辑**，尤其是最重要的业务逻辑——每条状态机、每个分支、每个失败路径都要有真实数据能触发，不是只种"一切正常"那一种。
2. **局部性测试**：有新需求时，能只对着一个组件（连同它的强依赖树）就把新逻辑测完，不需要先把全套 14 个组件都启动。
3. **组件部署前的完整链路测试**：包含依赖组件在内的一整条链路，都要有数据能撑起一次端到端的真实调用。
4. **项目部署前的全方位测试**：整个装配起来后，各个角度（权限边界、跨组件事件、TCC 补偿…）都要有数据能覆盖到。
5. **用户体验/演示时的"游乐场"效果**：只有几条数据的项目没法体验——真正能体现项目完整性的，是大量客户、各种角色、各种部门同时存在，用户点开随便一个页面都有东西可看、可查、可玩。

在这五条之外，还有几件事是"数据量大"本身不会自动带来、需要专门设计的：
- **数据权限维度要有真实存在感**：`org`/`owner`/`warehouse` 这些数据权限维度，只有真的存在多个部门/多个负责人/多个仓库的数据，"这个人看不到别人的数据"这类测试和演示才有意义——种子数据如果全挂在一个 admin 名下，数据权限这整块能力在演示里根本"看不见"。
- **量要能触发分页**：`List` 类接口强制游标分页（决策 53），种子数据量太小（比如 5 条）永远只有一页，这条契约的存在感也是零。
- **时间跨度要铺开**：部分数据应该分布在不同月份/跨越分区边界，"最近 N 天"这类查询、报表趋势图、分区维护逻辑才有真实场景可测，而不是所有东西都停在"今天创建"。
- **负向/边界数据同样要有**：`DISABLED` 客户、零库存产品、超期未结订单、被拒商机——失败路径的 UI 和逻辑不能只靠脑补，得有真数据能点开看。
- **数据要可发现**：量一旦上来，需要一份轻量的"种子数据一览"（哪个固定 `idempotency_key` 对应什么概念），不然新组件想复用别的组件的数据时无从查起，也容易在不知情的情况下起冲突——这是"种子数据是轻量契约"这条既有判据的自然延伸。**落地位置**：`infra/seed-data/README.md` 现有的"灌了什么"表格就是这份一览的雏形（组件/数据/数量/谁负责），随着各组件数据种类变多，这张表要跟着扩充到"列出每个有意义的固定 `idempotency_key` 对应的具体画像"这个粒度，不需要另开新文件——一个地方查得到即可，多一份文件就多一份要保持同步的负担。

**冗余是可以接受的，甚至是好事**：不同组件的种子数据如果概念上有重叠（比如两个组件各自造了一个"制造业客户"画像），不需要费力去重——反正都是假数据，冗余的代价趋近于零，而"数据量大、看起来完整"这件事本身就是目标之一，别的组件需要类似数据做测试时，直接复用现成的也比协调"谁该拥有这份数据"更省事。

**下游可以、也应该倒逼上游丰富数据，不是被动等**：如果一个组件测试/演示需要的数据组合（比如"一个属于华南部门、状态是 DISABLED 的客户"），上游组件的种子数据里还没有，正确做法是**直接去改上游组件自己的 `seed.sh` 把这个组合加上**，不是在下游这边绕开数据将就着测、也不是干等着"以后有人加"。这跟"上游丰富，下游才有得挑"是同一个判据的主动版——数据是假的，想让上游多造几种，改一下就是了，不需要走什么审批。
...
- **完整度要求**：种子数据要把这个组件有意义的状态都各来一条，不是只种欢乐路径——比如一个有状态机的组件，种子数据里每个状态至少各出现一次；一个有失败分支的组件，至少种一条能触发失败分支的样本；有数据权限维度的组件，至少造两个不同的 `org`/`owner`/`warehouse` 各自名下的数据（不然"看不到别人的"这类测试和演示都无从谈起）；量大到能真的触发一次分页；时间戳分布在不同月份而不是全部挤在"今天"。这样打开项目就能直接点开每一种情况看，而不是只能看到「一切正常」那一种。**这条要求也是链式调用能成立的地基**：下游组件的 `seed` 要在上游产出的数据上组合出自己需要的东西（比如 `crm-opportunity` 需要"一个 `ACTIVE` 客户+一个 `ACTIVE` 产品"这种组合），上游的种子数据够丰富，下游才有得挑；上游只种一条，下游能拼出的花样也只有一种。上游现有数据组合不出下游需要的东西时，直接去改上游的 `seed.sh` 补上，不要在下游将就。
- **弱依赖：能连就建关联数据，连不上就优雅跳过——但对"本地模拟/测试"这个场景，默认预期是"连得上"**。这里要分清两个不同的问题：「客户会不会买」是**真实部署**才有意义的问题，`make seed`/`make seed-data` 从不出现在部署脚本或 CI 里、纯手动本地触发（见下方"这不是什么"），根本碰不到客户的真实系统；而我们自己的开发副本迟早会把全部组件建完，所以对我们来说"这个弱依赖存不存在"几乎恒为"存在"，不需要当成常态去防。判据因此是：**能连就建，这是默认预期，不是"运气好才发生"的分支**；探测机制（同 `besdk.Endpoint()` 的二值返回）留着当防御性兜底——万一真的只装了这个组件的强依赖树、弱依赖没跟着装，探测不到就跳过那一步、只把自己分内的数据种完，不因为一个弱依赖缺失就整个失败退出，但这是兜底路径不是主路径。
- ⚠️ **种子数据的"完整"不包括一类东西：能进真实客户部署的凭据/身份数据**。这里有一条硬边界，安全相关，不是设计偏好：`make seed`/`seed-clean` 建的东西（假客户、假商机、`dev.superuser` 这类测试账号）**绝不能写进 migration**——migration 是每次部署都会跑的，包括真实客户的生产部署，写进去等于给每一个客户的系统里永久埋一个密码公开、权限覆盖一切的后门账号。这跟"生产用的 Casdoor 应用不许开 `password` 授权类型"是同一类风险。
  - **真正通用、任何部署都需要的基线/参考数据**（默认仓库、默认法人+基础科目表、基础计量单位这类）继续走 migration——这部分不是"测试数据"，是产品本身的一部分，`erp-inventory` 的 `WH-EAST`/`WH-SOUTH`、`erp-finance` 的默认法人与科目表、`mdm-product` 的基础计量单位都是这么做的，继续保持。
  - **真实部署"第一个管理员怎么来"这个鸡生蛋问题，答案不是种子数据，是配置驱动的自举**：`infra-authz` 的 `configSchema.bootstrapAdminSub` + `Start()` 里的 `EnsureBootstrapAdmin`——真实部署时，让第一个人登录一次拿到他真实的 `sub`，填进这个配置项，容器一启动就幂等授予 `authz_admin` 角色，不需要任何人手写 SQL，也不需要任何硬编码账号。⚠️ 这个机制只解决"authz 自己的管理权限"，跟本地测试要用的"持有全部权限键的超级测试角色"是两回事，不能互相替代——后者只能是 `make seed` 手动建的、绝不落 migration 的东西。
- ⚠️ **"撤销种子数据"不等于"逐行 DELETE"——有的组件只能靠整库重置**：`seed-clean` 的隐含假设是"这张表允许删行"，但像 `erp-inventory` 这类流水表本身就"只增不改"（AGENTS.md 已有的既有判据，出于业务/审计原因）——逐行删既做不到真正的干净复原（`BIGSERIAL` 序列不会因为 `DELETE` 回退，下次种的数据 id 会从一个很大的数字继续，不是真的回到"刚建完库"那个状态），又违背表本身"只增不改"这条设计原则。这类组件的正确工具是 `make db-reset`（`migrate down` 再 `up`）——**真正的重置，不是删除**：drop 掉整个 schema 的表再按迁移重建，序列真的从 1 开始，migration 播种的基线数据（默认仓库、默认法人这类）也会跟着重新落地。判据：**一张表如果只增不改，它就没有 `seed-clean`，只有 `db-reset`**；能安全逐行删的表两者都可以有，`seed-clean` 更轻量（不影响其它数据），优先用它。
- **种子数据本身是一种轻量契约，随版本变化要意识到下游可能在依赖它**——一旦有别的组件的 `seed` 反查过某个组件的固定 `idempotency_key`（比如 `crm-opportunity` 反查 `mdm-customer` 的 `seed-customer-1`），这个组件改自己的种子数据时就不能随手删/改这些已经被引用的 key（同 `registry/permissions.tsv`/events 契约的"只增不改"精神，只是这里是约定俗成不是门禁强制）。真出现需要调整的情况（比如某个上游组件种子数据结构变了，下游的 `seed` 跟着报错），**靠的是每个仓库自己的 git tag 去查历史**——不是只看 Docker 镜像版本号：`git show <旧tag>:scripts/seed.sh`/`git log -p --all -- scripts/seed.sh` 能看到某个版本的种子数据到底长什么样，比"猜镜像里跑的是哪版代码"可靠得多。这也是为什么"每次提交都要打带注解的 tag"这条纪律（`make version-check` 强制，不因为"只是文档改动"就放过）不只是为了 image tag 一致——它同时是这里排查/适配用的历史索引。
...
**跨组件测试分组：局部跑一个组件 + 它的强依赖树**

平时 `make test` 里，跨组件 L4 测试靠环境变量缺失就 `t.Skip`（如 `requireE2EEnv`）——不设 `MDM_CUSTOMER_GRPC_ENDPOINT` 就不跑。问题是平台不把 gRPC 额外端口映射到宿主机（导读第 1/14 条），宿主机的 `go test` 进程够不到 `mdm-customer:9090`，所以这些测试**在日常从来跑不起来**，跨组件验证一直只是"人手工搭好网络才跑一次"。

**`make test-cross REPO=<仓库名>`**（`infra/scripts/test-cross.sh`）补这个：读组件 `component.yaml` 的强依赖，给每条依赖起一个 `alpine/socat` 转发容器（宿主机 `127.0.0.1:2xxxx` ← 依赖容器的 gRPC 端口，走 brickkit 网络 DNS 名），把 `<DEP>_GRPC_ENDPOINT` 指过去，然后照常在宿主机上 `go test`（用宿主机 Go cache、`-race` 正常），跑完删转发器。**测试进程在宿主机，只有转发器是容器。**

- 依赖不必是全套 `brickkit up`——只要"这个组件连同强依赖树"在跑就行（§1.5 原则一）。这就是"局部性测试"：`make test-cross REPO=crm-opportunity` 只跑 crm-opportunity 自己的测试，打的是真实的 `mdm-customer`/`mdm-product`，别的组件在不在跑都不影响。
- 带过滤：`bash infra/scripts/test-cross.sh crm-opportunity -run TestCreateOpportunity -v`。
- ⚠️ 目标是"本组件调用它的依赖"（出边 gRPC）。"别的组件消费本组件发的事件"是另一回事（E1：同机 `brickkit up` 的真实兄弟容器会跟测试抢同一个 NATS subject），那类测试要单独 `docker stop` 兄弟容器，不在 `test-cross` 的射程内。
- 命名约定：新写的跨组件测试函数叫 `TestCross_<对方组件驼峰名>_...`（Python/TS 在函数名/`describe` 块保留前缀），配合 `-run`/`-k`/`-t` 能进一步只跑「某两个组件之间」。⚠️ 只对新测试生效，不强制回头重命名已有的——没有回归风险也没有紧急性，顺手改到就改。

#### W-8 · 扩展测试范畴（试运行中，逐项验证）

L1-L4 是稳定的四层。下面这几类是阶段三收官后讨论测试体系时提出的候选范畴，**逐项验证可行性**——能落地就转正（把测试要求写进 SOP-B/SOP-F 的对应步骤，或加一个 `make` 门禁），验证下来性价比不足就在这里留一行"XX：验证下来 <理由>，不做"，不要直接删干净（省得下次又有人重新提）。当前状态见每项标记。

| 范畴 | 测什么 | 现状 | 状态 |
|---|---|---|---|
| **权限/数据权限边界测试** | 原来想的"四条全测"拆清楚：无 token→401/无权限→403 是 `besdk.RequirePermission` 的职责，SDK 里测一次、`bare-route-scan` 保证每个端点都走这条路径，不需要逐组件重复测；真正逐组件才有的缺口只有"自己的数据看得到/别人的数据看不到"——范围过滤逻辑是每个组件手写的 | ✅ **已落地**：`be-acceptance` 新增 `data-scope-test-scan` gate（`gates/datascopetestscan.go`）——读每个组件 `assembly.yaml` 的 `data_scopes` 维度，检查测试文件里有没有至少一条"越权/超出范围被拒绝"形状的测试（判据：单独出现 `forbidden` 就算数，否则要求"授权/范围/越权/别人/他人"这类限定词跟"拒绝/看不到/查不到"成对出现）。真查出 `crm-opportunity`/`infra-notification` 零覆盖并补上了（各自 v1.0.1） | ✅ 已转正，接进 `make gates`。判据是"组件级至少一条"不是"每个维度各一条"——按维度关键字精度不够（`infra-workflow` 的既有测试从不点名具体维度），细粒度漏洞留给 W-6 人工审查 |
| **事件契约破坏性变更检测** | `contracts/events/*.json` 的 schema 演进符合"只增不删不改"（§3.10、决策 19） | ✅ **已落地**：`be-acceptance` 新增 `events-breaking-scan` gate（`gates/eventsbreaking.go`），把每个组件的 events JSON 工作区版本与其 git `main` 版本各自拍平成"每字段路径 + 类型 + subject 存在"的签名集，`main` 里的签名少一条就报违规（删字段/改类型/删 subject）；追加新字段新事件只产生新签名不触发。已接进 `make gates`（`make tier0` 会带着跑）。没写成通用 JSON Schema diff——events JSON 是自定义的 envelope+events 结构，专门写一个反而更准更简单 | ✅ 已转正。仅接了 `make gates`（装配级），没逐个改 12 份组件 Makefile 的 `contract-check`——events 文件很少动，装配级门禁足够，同 `TestCross_` 命名约定一样不强制回填 |
| **前端视觉回归测试** | AI 分批生成的页面之间视觉一致（间距/密度/组件用法），对应决策 113/114 点名担心的"视觉不统一" | 改由 SOP-F 的 **FE-4** 承接（不再单独放这里管）——机制定了（Playwright 截图对比，复用 FE-3 的固定装态），实现跟 FE-3 一起推迟到前端功能基本开发完 | ➡️ 移交 SOP-F，见该节 FE-3/FE-4 |
| **跨组件测试能局部跑起来** | 一个组件 + 它的强依赖树就能真跑那些平时被 `t.Skip` 的 L4 跨组件测试 | ✅ **已落地**：`make test-cross REPO=<仓库名>`（`infra/scripts/test-cross.sh`）——socat 转发器把依赖容器的 gRPC 端口桥到宿主机，测试照常在宿主机跑。实测 `erp-sales` 的 `tcc` 包从"0.005s 全 skip"变成"3.4s 真跑完整 TCC 链"。见上方"跨组件测试分组"小节 | ✅ 已转正（装配级 `make test-cross`）。`t.Parallel()` 暂缓——当前 `make test` 已经很快（跨组件测试全 skip、Go 默认按包并行），真正的价值在"让 L4 能 routine 跑"而不是并行，这条已经解决 |
| **SDK 并发正确性测试** | `be-sdk-*` 三个被全部组件共用的 SDK，它们的横切原语（Outbox pump、幂等表操作、连接池）在"多实例/并发"场景下的正确性——L1-L4 全是单组件视角，测不到"同一个 SDK 原语被多个实例同时用" | 首例已落地：`be-sdk-go@v0.2.5` 修 `StartOutboxPump` 重复投递 bug（踩坑记录 C15）时新增了 `TestPumpOnce_两个实例并发认领同一批事件_每条只发一次` 等三条，活在 `be-sdk-go` 自己的仓库里 | ✅ 首例已验证：这类测试确实抓到了一个真实生产级 bug，模式可行。剩下的是把 `be-sdk-python`/`be-sdk-ts` 的同类原语也照这个模式补上 |

#### W-9 · 三种运行粒度：全量 / 组件 / 组件内子集，后端前端通用

这一节回答"什么时候跑哪一种测试"——不分语言，Go/Python/TS/前端都是同一套三种粒度，只是命令不同：

| 粒度 | 什么时候跑 | 后端怎么跑 | 前端怎么跑 |
|---|---|---|---|
| **① 全量** | 部署前、阶段收尾、给整个装配一次全量验收 | `make gates` + 每个组件 `make test`（`make tier0` 之下） | `pnpm test`（根目录，跑全部 package） |
| **② 组件完整** | 一个组件/一个功能写完了 | `make -C components/<scope>/<name> test`；`make test-cross REPO=<x>` 把平时被 skip 的跨组件 L4 也跑起来（W-7） | `pnpm --filter <pkg> test` |
| **③ 组件内子集** | 迭代开发某个新功能，想确认"这部分还没被我搞坏" | `go test -run <正则>` / `pytest -k <表达式>` | `vitest <文件/pattern>`（watch 模式按依赖关系自动只重跑相关的） |

③ 不需要额外工具——三种语言的原生测试运行器都自带按名字/路径过滤，唯一要做的是**让测试名字可预测**：L1-L4 的层前缀（W-2）、`TestCross_<对方组件>` 的跨组件前缀（W-7）都是为了让"跑一个正则就能命中我想要的那一撮"这件事成立——命名混乱的话，这一层粒度就名存实亡，只能靠人肉翻文件找测试。

#### W-10 · 允许整体重写，不只是增量打补丁

持续开发的过程里，有些东西会因为当时判断不周、或者版本一路迭代下来方向本身跑偏了，事后看不是"改小块能修好"的状态。**AI 写代码的成本结构跟人工完全不同**——重写一整份文件的成本可能比"读懂三层历史补丁再小心翼翼地加第四层"还低，所以没必要死守"能不动就不动"这条对人类工程师才成立的经验法则。

**判据（什么时候该整体重写而不是打补丁）：**
- 同一块逻辑已经被打了好几次方向不同的补丁，现在要改哪怕一行都得先在脑子里叠好几层历史才搞得清当前状态到底是什么。
- 不是发现了一个 bug，是发现"当时那个设计判断，现在看本身就是错的"——继续在错的地基上加东西，只会越补越歪，早晚要重写，不如早点动手。
- **种子/演示数据类内容判据可以更松**：这类内容本来就是假数据，没有"迁移历史数据"的负担，重写成本约等于"删了重建"——只要现状不是当前想要的样子，直接整个重新设计一遍数据覆盖面，比一条条加数据去凑更快也更干净。这也是这次"数据种类要多、允许冗余"这些新规矩落地时的实际工作方式。

**流程：**
1. 动手前，把要重写的文件复制一份到本地 scratchpad（会话的临时目录，不进 git）当参照——**不是为了留档**（git 历史本来就留着每一个旧版本，`git show <tag>:path`/`git log -p` 随时能查），纯粹是写新版本时手边能对照旧逻辑，不用来回切 `git log`。
2. 重写产出**单独一次 commit**，message 里说清楚"为什么重写"而不只是"改了什么"——尤其是非数据类的代码重写，方便以后 `git log` 排查"这段逻辑什么时候被推倒重来过、当时是为了什么"。
3. 重写后照样跑一遍这个组件完整的验收门禁（`make all`/`make gates`/`make test`），**门槛不因为"是重写不是打补丁"而降低**——重写引入新 bug 的风险不比增量修改低，改动面通常更大，反而更该多验证一遍。
4. 验证通过、提交完成后，删掉 scratchpad 里的参照副本——它的使命结束了，真正的历史记录在 git 里，不需要在临时目录留一份占地方。
5. 如果重写的起因是"发现以前的方向错了"，值得留一笔记录：技术层面"看起来对、实际错"的教训记 `docs/dev/实测踩坑记录.md`；设计判断本身要更新的记对应 `docs/design/<仓库名>.md`——不然下次又会照着旧文档把错的方向重新做一遍。

**这次"数据种类要多、允许冗余"等新规矩要落地到各组件时，判据 2（种子数据类可以更松）会是主要适用场景**：给某个组件"加数据"不一定是在现有 `seed.sh` 后面追加几行，更可能是把整份 `seed.sh` 按新判据重新设计一遍——这是预期中的正常工作方式，不是"没控制好改动范围"。
...
#### F-测试 · 前端测试分层（FE-1 ~ FE-4）

这一节是 SOP-W 测试分层（W-2）+ 运行粒度（W-9）在前端的对应版——工具换了，判据和粒度概念是同一套，不重新发明。三种运行粒度同 W-9：① 全量 `pnpm test`（根目录）、② 单个 app/package `pnpm --filter <pkg> test`、③ 组件内子集用 Vitest 原生的文件/pattern 过滤（watch 模式还会按依赖关系自动只重跑相关的）。

⚠️ **现状核对（先说清楚缺什么，不是从零设计）**：`apps/pc`/`packages/ui-kit-pc`/`packages/design-tokens` 已经有 Vitest 单元测试，质量不差——`apps/pc/test/http.test.ts` 真的在测第 16 条"`401 token_stale` 静默刷新重试一次"这条规则，不是摆设。真正缺的是三件事：`packages/api-client`（本节原计划就该有、`frontend-standard` Task 12 落地时漏建的包）从没真正建出来，`apps/pc/src/api/*.ts` 全是手写类型；Playwright 只在 Task 12 手动跑过一次，从未固化成仓库资产；`apps/mobile` 零测试工具。

| 层 | 测什么 | 工具 | 现状 |
|---|---|---|---|
| **FE-1 单元/逻辑测试** | 纯函数、composable、store，mock 掉网络请求 | Vitest | ✅ 已在用（`http.test.ts`/`oidc.test.ts`/`theme.test.ts` 等），新功能继续按这个模式补，不受下面的推迟决定影响 |
| **FE-2 组件测试** | `ui-kit-pc`/`ui-kit-mobile` 里被多个业务页面复用的共享组件——改坏一个等于改坏所有引用它的页面 | Vitest + `@vue/test-utils` | 部分已有（`BeTable.test.ts`/`pageTemplates.test.ts`），`ui-kit-mobile` 还没有，随它补组件时一起补，同样不受推迟决定影响 |
| **FE-3 端到端测试** | 真浏览器 + 真后端（`brickkit up`）。PC 与移动端用**同一套** Playwright，切 device profile 区分，不需要模拟器——`apps/mobile` 编译目标是 Uni-app 的 **H5** 构建，是网页不是原生 App，只有 uni-app 原生桥接 API（扫码/相机类）测不到，先人工验证 | Playwright | 🔲 **推迟到前端功能基本开发完再统一做**（用户拍板：AI 开发这种迭代节奏下，页面还在反复改时就配 Playwright，每次调试都要截图/读 DOM，token 成本和收益不成比例；等页面稳定下来一次性补，不是忘了）。下面先把骨架定死，到时候直接填 |
| **FE-4 视觉回归测试** | 页面间距/密度/组件用法一致（决策 113/114 点名担心的"视觉不统一"），截图对比 | Playwright 自带截图对比 | 🔲 同 FE-3 一起推迟——机制上是 FE-3 的一种断言方式，不是独立的测试运行器，不单独起一遍浏览器 |

**FE-3/FE-4 推迟不等于不设计**——骨架先定，到时候直接填，不用再设计一遍：

- 目录：`apps/pc/e2e/`、`apps/mobile/e2e/` 各自独立；一份 `playwright.config.ts`（放 `packages/` 下，两个 app 共用），`projects` 字段区分 `pc` / `mobile-h5` 两个 device profile。
- 命名延续 L1-L4/`TestCross_` 那条判据：按"验证的业务流程"命名，不按"点了哪个按钮"命名，方便以后用 `--grep` 做③粒度的子集筛选。
- FE-4 复用 FE-3 的固定装态：每条 E2E 流程走到关键页面时顺手 `expect(page).toHaveScreenshot()`，基线随改动一起更新，不单独维护一套"纯截图"的测试文件。

**契约漂移：不靠测试堵，靠让它不可能发生**——`apps/pc/src/api/customer.ts` 这类手写类型（注释自己写"逐字对应契约文件"，但没有任何机制保证不漂移）不该靠多写一层测试去追，那是把一个能从根上消除的问题变成反复要交的维护税。正解是把 `packages/api-client` 真的建起来：从 `contracts/*.openapi.yaml`/BFF 的 GraphQL schema 生成 TS 类型 + 请求函数，手写的 `apps/pc/src/api/*.ts` 逐步换成用它。这跟"推迟 E2E"性质不同——是一次性建设成本，装好之后每加一个新接口零额外成本（`tsc` 编译本身就是这层检查，不需要另外跑东西），不受上面的推迟决定管。🔲 待实现，优先级：常规下次碰 `frontend-standard` 时顺带做，不必须专门起一轮任务。
