...
| 范畴 | 测什么 | 现状 | 状态 |
|---|---|---|---|
| **权限/数据权限边界测试** | 每个声明了 `data_scopes` 的组件，对同一个端点固定测四条：无 token→401、有 token 无权限→403、有权限但数据属于别人→查不到、有权限且是自己的→查得到 | 现在是随事故散在各组件补回归测试（`infra-workflow` 的 fail-open 漏洞、决策 116 讨论的 OR-vs-toggle） | 🔲 待验证：能不能做成一个各组件都套的清单/helper，而不是每次靠人记得 |
