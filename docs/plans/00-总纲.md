...
| **事件契约破坏性变更检测** | `contracts/events/*.json` 的 schema 演进符合"只增不删不改"（§3.10、决策 19） | **确认空白**：`make contract-check` 跑的 `buf breaking` 只认 `.proto`，不碰 events JSON；`tools/be-acceptance` 也没有对应 gate；设计书/本文件的已知风险清单里从没讨论过这个缺口。⚠️ `docs/plans/02-阶段二-验平台.md:248` 有一句"`events/*.json` … `buf breaking` 绿"，那句是错的（`buf` 不处理 JSON）。人工评审（W-6）是目前唯一防线 | 🔲 待验证：找/搭一个 JSON Schema 破坏性 diff 工具接进 `make contract-check`；先顺手修正 02 阶段计划里那句错的话 |
