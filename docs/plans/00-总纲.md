...
- **弱依赖：探测可达性，能连就建关联数据，连不上就优雅跳过**——弱依赖不像强依赖那样"启动这个组件必然启动它"（客户可能没买那个弱依赖），所以不能像强依赖一样无条件链式调用它的 `seed`；但也不是"完全不管"：真实部署时，只要客户买了两边，`brickkit up` 就会把它们一起启动，`seed` 完全可以趁这个机会把跨组件的关联数据也建出来，让演示更完整。判据跟运行时代码对弱依赖的既有判据完全一致（`besdk.Endpoint()` 的二值返回）：`seed.sh` 探测一下这个弱依赖的健康检查，`ok` 就多做一步把关联数据建好，探测不到就跳过那一步，只把自己分内的数据种完，不因为一个弱依赖缺失就整个失败退出。
