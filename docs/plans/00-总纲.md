...
⚠️ **消费者测试自己构造/发布事件时，用测试私有的 subject，不要用真实生产 subject。** 同一台机器上 `brickkit up` 真实跑着的容器订阅的正是那个生产 subject，`besdk.Consume` 走的是 NATS 核心订阅（广播，不是竞争消费）——真实容器会跟本地测试的临时订阅者同时收到消息，谁先抢到 `event_inbox` 的认领谁的产出就留在数据库里，断言可能读到的是别人的结果（踩坑记录类别 E 的 E1/E2）。`besdk.Consume` 的 `fn` 不拿 `ev.Subject` 做业务判断，换成 `fmt.Sprintf("test.%s.%d", base, time.Now().UnixNano())` 这类测试私有名字完全安全，真实容器压根收不到。⚠️ 这条只适用于"测试自己发布事件去触发消费者"；验证"业务逻辑真的发到了正确的生产 subject"这件事本身的测试（如 `TestMarkWon_...`）必须用真实 subject。
