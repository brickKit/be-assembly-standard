...
11. **视觉方向「克制专业型」**：只改 seed token 的主色、开 `compactAlgorithm`、系统字体栈（不引入需联网或内嵌的自选字体）、业务页面**禁止硬编码颜色值与间距数值**（值全在 `packages/design-tokens`）。⚠️ **「好看」在本项目里的具体含义是「一致」**——61 个组件的页面由 AI 分批生成，最容易毁观感的不是配色不够大胆，而是「这一页间距 16、下一页 20」（§12.6.5）。
