...
这也是 SOP-D 里「参考实现」那一节要写清 **repo + 模块路径** 的真正原因：它不只是给人看的出处，更是**给 AI 的源码指针**——写不出某段逻辑时，先去读那个模块，再回来实现。

#### R-1 · ⚠️ 三条许可证纪律（违反会让整条 Fork 机制在法律上失效）

我们的交付模型是：`open_standard` 标准件 100% 开源 → 客户需要定制时 Fork → **`customer_fork` 闭源且归客户所有**（设计书 §3.2）。

而主流开源 ERP 几乎全是 copyleft。**把 copyleft 代码复制进标准件，客户 Fork 就是衍生作品，必须同样开源——「闭源且归客户所有」当场失效。**

| # | 纪律 | 说明 |
|---|---|---|
| **R-1a** | **读设计、领域模型、状态机、字段设计——永远安全** | 学"库存流水为什么必须不可变"、"会计期间为什么要锁"、"UoM 换算因子放哪张表"，这是知识，不是代码。**主要就靠这一条** |
| **R-1b** | **复制代码只限许可证宽松的项目**（Apache-2.0 / MIT / BSD），且必须在 `NOTICE` 里保留原始版权与许可证声明，并在设计计划第 8 节写明抄了哪个文件的哪一段 | 大型 ERP 里只有 **Apache OFBiz 是 Apache-2.0** |
| **R-1c** | **严禁把 GPL / LGPL / AGPL 项目的代码复制、翻译、或逐行改写进任何组件** | 「用 Go 重写一遍 Odoo 的那个函数」**同样是衍生作品**。这是最容易自我说服的一条——「我只是参考了它的算法」和「我把它翻译成了 Go」中间那条线，法务上比直觉里窄得多 |

⚠️ **上表里的许可证以各项目仓库当前的 `LICENSE` 文件为准，开工前必须现场复核。** 开源项目改许可证不是罕见事（近几年就有多起）。**本文档里的许可证标注是写作时的记录，不是承诺。**
...
|---|---|---|---|---|---|
| Odoo | 17.0 | `addons/product/models/product_uom.py` | UoM 换算因子的表设计与舍入策略 | LGPL-3 | **仅读设计**（R-1a） |
| Apache OFBiz | 18.12 | `applications/product/entitydef/entitymodel.xml` | Product/UoM 实体关系 | Apache-2.0 | **仅读设计**（若后续抄代码需补 NOTICE） |
...
⚠️ **「用法」列只有两个合法值**：`仅读设计` 或 `已复制代码（NOTICE 已补 + 许可证兼容）`。**没有「参考了算法然后自己写的」这个选项**——那一栏要么是前者（你只读了设计），要么是后者（你在翻译代码）。含糊其辞的地方就是法务风险所在。
...
「更容易」的衡量标准很具体——**一个只有设计模式基础、不懂我们业务的人（或一次新开会话的 AI），能不能只读一个文件就明白某一条规则怎么加进去。**
...
| **为了"以后可能要扩展"而提前抽象** | 设计书 §1.1 的原则是**拿不准不拆**（决策 9）：胶水成本是实打实的，而"以后"往往不来。等第二个实现真的出现时再抽——那时你才知道该沿哪条线抽 |
...
**判断拆对了没有，一个动作**：让一个没读过这个组件的人（或一次新开会话的 AI）加一条新折扣规则。**如果他只需要新建一个文件 + 在 `pricing.go` 的注册表里加一行，就是拆对了。** 如果他还要改别的规则文件，说明链的执行顺序或数据流没设计干净。
...
| 8 | **参考实现**：看了哪个项目的哪个模块、借鉴了什么、许可证、是「仅读设计」还是「已复制代码」（**开工前就要填**，见 SOP-R） | §3.2 |
...
| # | 设计书原本 | 已补成 | 为什么 | 回写到哪里 |
|---|---|---|---|---|
| 6 | 只在 `asset` 表里写了 `open_standard` 是「100% 开源」、`customer_fork` 是「闭源且归客户所有」，**没有任何参考实现纪律，也没写我们自己用哪个许可证** | 新增 **§3.2.1 参考实现纪律**：先看现实开源 ERP，但**读设计永远安全 / 复制代码只限 Apache-2.0·MIT·BSD / 严禁复制或翻译 GPL·LGPL·AGPL 代码**（「用 Go 重写一遍」同样是衍生作品） | 主流开源 ERP 几乎全是 copyleft（Odoo LGPL-3、ERPNext/Tryton GPL-3、SuiteCRM/Twenty AGPL-3）。抄进来，`customer_fork` 的「闭源且归客户所有」在法律上当场失效 | §3.2.1、决策 102 |
| 7 | 全书没有关于设计模式的任何口径 | 「不强制、但适合就必须用」，判据是「更容易被读懂和扩展，还是更难」；已确定要用的九处列在总纲 §4 SOP-P | 一个长文件拆成结构化的多个短文件之后，只要有设计模式基础就能快速理解并扩展——对每次新开会话的 AI 更是如此。反过来，纯 CRUD 硬套模式是更糟的结果 | 决策 103（细则在总纲 §4 SOP-P，因为它会随开工进度补充） |
...
| 10 | **⚠️ 我们自己的组件用哪个开源许可证，尚未决定** | 决定了「客户 Fork 件能不能闭源」这件事在法律上成不成立，也决定了能不能接受外部贡献。**必须在第一个仓库 push 之前定**——事后改许可证需要所有贡献者同意 | **阶段一 CP-1 之前**（那一步就要创建 `be-ops` / `be-acceptance` / `be-sdk-go` 三个仓库）。推荐 **Apache-2.0**：允许客户 Fork 件闭源、带专利授权、与 OFBiz 兼容可直接借用其代码 |
