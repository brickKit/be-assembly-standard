...
6. **技术栈锁定**：PC 端 Vue3 SPA、移动端 Uni-app（Vue3），TypeScript 贯穿，**坚决排除 React**（决策 79）。契约生成的 Hooks 用 `@tanstack/vue-query`（设计书 §3.9 旧版写的是「React Query Hooks」，已更正）。前端与 `infra-bff-mobile` **都不进任何外壳**（§13.5），所以 TS 侧没有 §K 那套模块入口契约，栈锁定只为一致性。
