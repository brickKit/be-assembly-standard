...
| **前端视觉回归测试** | AI 分批生成的页面之间视觉一致（间距/密度/组件用法），对应决策 113/114 点名担心的"视觉不统一" | `frontend-standard` 有 Playwright 端到端测试，验的是功能对不对，没有截图对比 | 🔲 待验证：Playwright 自带截图对比能力，评估基线怎么管、CI 缺失下本地怎么跑 |
