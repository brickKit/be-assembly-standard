...
#### 外壳三 · Go 基建与外部通道（22 个，HTTP 8200–8222 / gRPC 9200–9222）
...
| 22 | `infra-authz` | `infra/authz` | 8222 | 9222 |

⚠️ **`infra-authz` 排在最后而不是插在 infra 段中间，是刻意的**：`ports.tsv` 只增不改（§3.5.1.1），8221 已经给了 `infra-iam-keycloak`，所以新组件只能往后取 8222。**不要为了「看起来整齐」去重排已发布的端口。**
...
| 11 | `infra-authz` | default | Go | 三 | 8222/9222 | `infra_authz` | **阶段三** |
