...
> **前置：** 先读 [`00-master-guide.md`](../standards/00-master-guide.md)。总纲里的**全局约束**（A–J 十条）、**端口册与 schema 册**、
...
⚠️ **两处实测踩坑**（`${VAR}` 只能整值替换不能嵌字符串中间、`brickkit_db`/`casdoor` schema 在 `make up` 时还不存在），细节见 [`docs/dev/field-tested-pitfalls-log.md`](../dev/field-tested-pitfalls-log.md) A4。这里只给改法：
...
⚠️ **本任务实测发现两处 Task 1 遗留的 bug**（`docker compose config` 只检查语法、从不启动容器，所以带着坏配置也能通过——直到这里第一次 `make up`/`make obs-up` 才暴露），细节见 [`docs/dev/field-tested-pitfalls-log.md`](../dev/field-tested-pitfalls-log.md) A1（nats `--max_payload` 不是合法 CLI 参数）、A2（otel-collector/loki/tempo 三个镜像没有 shell，健康检查必然失败）、A3（Loki 的启动延迟不是 bug）。这里只给改法：
...
⚠️ 原计划这里写了先解决 `my-postgres`/`my-rustfs` 端口冲突，但那在 Task 1 已经解决了（[`field-tested-pitfalls-log.md`](../dev/field-tested-pitfalls-log.md) D1）。直接：

```bash
make up
```

⚠️ **已实测：第一次跑会被 Task 1 遗留的 nats 配置 bug 挡住**（[`field-tested-pitfalls-log.md`](../dev/field-tested-pitfalls-log.md) A1），报 `container be-nats is unhealthy`。处理方式：
...
⚠️ `make rustfs-down` 这个目标不存在——默认资源不该有单独的 `-down` 目标（见 [`field-tested-pitfalls-log.md`](../dev/field-tested-pitfalls-log.md) D2）。改成直接测脚本本身：
...
内容照[总纲](../standards/00-master-guide.md) §2.1 的六张表逐行抄，包含带外容器段。格式：
