...
## 8. End-User-Facing Documentation Is a Separate Locale Axis, Not §6's Mirror System

§6's Tier-1/Tier-2 split exists for exactly one reason: AI reading speed on internal, dev/architect-facing documents. English is the default there purely because that's what the AI reads fastest; the `docs/zh/` mirror exists purely because this project's human maintainer communicates in Chinese only (see the note at the top of `AGENTS.md`). Nothing about that system is a statement about the shipped product.

Documentation a real end user of the shipped product reads — starting with the deployment handbook, and eventually a customer/admin manual — is a different concern. The product itself supports English and Chinese as end-user languages and defaults to English (2026-09-18, when the deployment handbook was first given a genuine English version). Documentation for that audience needs real parity in both languages, matching the product's own supported locales — it is governed by this section, not by §6's tiering, and a document can be pulled into this section regardless of whether it would otherwise read as Tier 1 or Tier 2 by §6's own test.

**Where these live**: `docs/<category>/{zh,en}/<doc>.md` — the language folder is nested *inside* the doc category (e.g. `docs/ops/zh/deployment-handbook.md`, `docs/ops/en/deployment-handbook.md`), not a top-level `docs/zh/`/`docs/en/`. This is deliberate, not a stylistic preference: top-level `docs/zh/` is already claimed by §6's AI-comfort mirror, and that mirror is not just documentation — `tools/be-acceptance`'s `bump-version` tooling (SOP-W-11) hardcodes `docs/zh/AGENTS.md`'s exact path to keep the component roster table in sync on every version bump. Relocating that mirror is a real cross-repo change (Go source in a separate submodule, its own version bump, a submodule-pointer update in this repo) and is explicitly **not** part of this section — see the target structure below.

**Filename convention**: both language versions of the same document share one identical, ASCII filename, differing only by which `{zh,en}` folder they sit in (`deployment-handbook.md` in both `zh/` and `en/`, never a Chinese filename on one side and an English one on the other). This differs from §6's mirror convention (which reuses whatever filename the Tier-1 English original already has) because these documents are frequently authored in Chinese first in practice, and a stable ASCII filename is what keeps the two locale folders' listings visibly paired.

**Keeping parity**: whichever language is edited first for a given change, update the other language's file in the same commit — same discipline as §6's mirror, applied for a different reason (matching the product's own locale support, not AI reading speed). Each file links to its counterpart near the top (`*[English](../en/<doc>.md)*` / `*[中文](../zh/<doc>.md)*`).

**Target structure, not yet executed**: `docs/` should eventually reorganize into top-level `docs/{dev,zh,en}/` — `dev/` holding everything §6/§7 currently governs plus the rest of this repo's dev-facing content that's still sitting loose (`docs/standards/`, `docs/design/`, `docs/plans/`, `docs/retrospectives/`, `docs/arsenal-maintenance-handbook.md`, and `docs/dev/`'s own existing files), with §6's `docs/zh/` mirror relocating to `docs/dev/zh/` once that move happens — freeing top-level `docs/zh/`/`docs/en/` for the full set of end-user-facing documentation, of which `docs/ops/` would become one category alongside others still to be written (a customer/admin manual, per `docs/README.md`'s "still to be written" row). That migration is deliberately deferred: besides the `be-acceptance` tooling dependency above, it touches every component's own `AGENTS.md` (each references `docs/design/<repo>.md`) and `infra/scripts/docs-check.sh`. Until it happens, `docs/ops/{zh,en}/` is the only place following this section's convention; everything else in `docs/` still follows §6/§7, or is untouched Tier-2 Chinese-only content.
