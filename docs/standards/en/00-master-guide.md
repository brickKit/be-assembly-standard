...
⚠️ **The count "11" is the original, fixed numbering — not the number of outputs `be-ops` still produces today.** Outputs 4/7/8 (all three, merged-deployment-specific) have since been retired one by one as brickKit's own `servedBy` mechanism grew native support for exactly what each one used to hand-roll — see each row's own note below. The numbers themselves are kept stable rather than renumbered, since other documents (this one included, §14.1.2/§14.2.2) cite outputs 9/10 by number.
...
| 4 | ~~Shell-merge configuration: which components go in which shell, port allocation, migration execution order.~~ **Retired** — brickKit v0.4.2 added the native `BRICKKIT_SERVED_MEMBERS_CONFIG` env var, computed from data the injection engine already builds for every running component; the shell binary parses it directly, no generator step needed | The platform provides no support for merged deployment (§13.6) — **true when this row was written, no longer true after v0.4.2** | **Phase 4**, retired Phase 4b addendum Task 0.6 (`docs/plans/05a-迁移到servedBy.md`) |
...
| 7 | ~~Per-shell environment-variable table: a same-shell dependency → `http://127.0.0.1:<port>`; a cross-shell/standalone-container dependency → `http://<host address>:<port>`~~ **Retired** — brickKit's native `servedBy` merge rewrites every `*_ENDPOINT` address directly into the shell container's own environment at generation time | The platform only injects into the containers it generates itself, and after merging those containers don't exist (§13.8) — **true when this row was written, no longer true once `servedBy` shipped** | **Phase 4**, retired Phase 4b addendum Task 0.2/0.3 (`docs/plans/05a-迁移到servedBy.md`) |
| 8 | ~~shell-compose's `depends_on`: start order between shells~~ **Retired** — brickKit's native `servedBy` merge computes inter-shell start order itself; the hand-written `infra/shell-compose.yml` orchestration it replaced is deleted | The platform only sequences the containers it generates itself (§13.8) — **true when this row was written, no longer true once `servedBy` shipped** | **Phase 4**, retired Phase 4b addendum Task 0.5 (`docs/plans/05a-迁移到servedBy.md`) |
...
⚠️ **What the tool itself deliberately does *not* do**: `git commit`/`tag`/`make image`/`push`. Editing files in bulk is a safe, reviewable, git-revertable operation; pushing a tagged, rebuilt image to several separate component repos is exactly the class of "visible to others, hard to reverse" action this project has always kept a checkpoint in front of (real-machine verification, `make gates`, reading the diff) — baking that into the same program that does bulk file edits would mean a bug in one code path risks the other. The workflow stays two-phase on purpose:
...
2. **Finalize per component, in the order the tool printed** (its printed order is root-changes-first, then whatever they pulled in — purely for readability; the file edits themselves don't depend on doing this in any particular order, since every new version number was already decided in step 1). For each: run that component's own tests, `git commit` + `git tag -a` + `make image`, `git push` (commit and tag). Then commit the parent repo (`brickkit.yaml`, both `AGENTS.md` files, the regenerated `.brickkit/manifests`), verify with a real `brickkit up` pass, `brickkit down`, and push.

**This project is developed end-to-end by an AI**, so "a checkpoint in front of it" doesn't have to mean a human types the git commands by hand every time — it means *whoever* executes the commit/tag/push step keeps exercising judgment on each one (does this commit message actually describe what changed, does this cascade look sane, is this really nothing but a version bump or did a real behavior change sneak in), rather than a program blindly chaining the two risk classes together with no judgment in between. That full step 2 sequence — including how to word the commit message, when a `bump-version`-computed cascade looks wrong enough to stop and investigate instead of applying it, and when a batch stops qualifying as "just a version bump" and needs to go through the normal development-review rhythm instead — is written up as the **`version-bump-ship` skill** (`.claude/skills/version-bump-ship/`), together with an optional discovery step (`infra/scripts/list-unshipped-components.sh`, for when it isn't already obvious which components this session actually touched — it lists every component repo whose `HEAD` sits ahead of its own latest tag, the same signal `make version-check` treats as a gate failure, read here instead as "candidates to review, not everything on the list is necessarily today's batch"). The skill exists so this rhythm doesn't have to be re-derived from scratch each time it's used — not so it can be run with less judgment than writing the code itself required.
