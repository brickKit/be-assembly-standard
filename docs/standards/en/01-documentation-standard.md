...
Every component-level document — the four governed by this section: `docs/design/zh/<repo>.md`, `README.md`, `docs/手册.md`, `AGENTS.md`+`CLAUDE.md` — must be checked against two distinct readers before it is considered finished:
...
### 4.1 The Design Document (`docs/design/zh/<repo>.md`) — written before work starts, in the assembly repository
...
- **A multi-file documentation category** gets a nested `docs/<category>/{zh,en}/<file>.md` folder pair, both language versions sharing the identical filename: `docs/standards/en/00-master-guide.md` / `docs/standards/zh/00-master-guide.md`, `docs/design/en/erp-sales.md` / `docs/design/zh/erp-sales.md`, `docs/ops/en/deployment-handbook.md` / `docs/ops/zh/deployment-handbook.md`.
- **A single standalone file** not part of a larger category gets a flat sibling next to it, named `<name>.zh.md`: `AGENTS.md`/`AGENTS.zh.md` at the repo root (this one specifically cannot move into a folder — Claude Code's own tooling requires `AGENTS.md` to sit at the repo root), `docs/arsenal-maintenance-handbook.md`/`.zh.md`, `docs/dev/field-tested-pitfalls-log.md`/`.zh.md`.
...
- `AGENTS.md`, `docs/standards/*`, and `docs/design/`'s cross-component material (`_replaceability-map.md`, `README.md`) are read by an AI session in full, where reading speed is the deciding factor — **English is canonical**.
- `docs/design/zh/<component>.md` (the 14 per-component design plans) — **Chinese is canonical**, simply because that's what they were authored in and nobody has translated them yet (see the known gap below).
- `docs/ops/` — canonical is whichever language was authored first for a given change; English and Chinese end users are equally real, so there's no default direction, only "keep both in sync."
...
**What stays outside this system entirely, Chinese-only, un-migrated**: `docs/plans/`, `docs/retrospectives/`, `docs/design/_调研记录/` (the long-form research dossiers behind a phase's design plans — a phase's design plans themselves condense the relevant findings, so this is genuinely a different, rarely-revisited document), and `docs/dev/`'s own narrative pitfalls-log entries within `field-tested-pitfalls-log.{md,zh.md}` (the file itself is bilingual per the pattern above; its *content* stays a mix of entries, none of them rewritten for style). These are execution-process narrative — written once, cited occasionally, never read wholesale as ongoing reference — and the cost of full bilingual parity for them isn't judged worth paying. `docs/design/_模板.md` is an authoring template for whoever writes the *next* design plan, not itself reference content anyone reads, so it stays a single file with no language pair at all. **Historical records in any of these are never rewritten for a path change — only relocated if the file itself physically moves.**
...
⚠️ **"Parity" means the prose, not the literal bytes of every relative link.** Two files in a `{zh,en}/` pair sit at the same depth from the repo root, so a link that resolves in one resolves in the other *only if what it points at is reachable at the same relative depth from both* — this must be re-verified after any structural move (a file relocating, a category gaining a language pair for the first time), not assumed to still hold. This is exactly what bit the September 2026 migration that first split `docs/standards/`/`docs/design/` into `{zh,en}/` pairs: the English files moved one directory deeper than their Chinese counterparts already sat at, and every relative link reaching outside the immediate folder needed its `../` count re-derived by hand, file by file.

**⚠️ Known gap**: the 14 component design plans under `docs/design/zh/` do not have English counterparts yet — `docs/design/en/` currently holds only `README.md` and `_replaceability-map.md`. Translating all fourteen is tracked as a separate, substantially larger follow-up, not assumed done by anything in this section.
