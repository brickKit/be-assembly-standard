...
Every component-level document — the four governed by this section: `docs/dev/design/<repo>.md`, `README.md`, `docs/手册.md`, `AGENTS.md`+`CLAUDE.md` — must be checked against two distinct readers before it is considered finished:
...
### 4.1 The Design Document (`docs/dev/design/<repo>.md`) — written before work starts, in the assembly repository
...
- **A multi-file documentation category** gets a nested `docs/<category>/{zh,en}/<file>.md` folder pair, both language versions sharing the identical filename: `docs/standards/en/00-master-guide.md` / `docs/standards/zh/00-master-guide.md`, `docs/ops/en/deployment-handbook.md` / `docs/ops/zh/deployment-handbook.md`.
...
- `docs/ops/` (including `_replaceability-map.md`, see below): canonical is whichever language was authored first for a given change; end users in either language are equally real, so there's no default direction, only "keep both in sync."
...
**`docs/dev/` is the single home for everything a real end user has no reason to read**: `docs/dev/design/` (the 14 per-component design plans, its own `README.md`, `_模板.md`, `_调研记录/`), `docs/dev/plans/`, `docs/dev/retrospectives/`, and `docs/dev/field-tested-pitfalls-log.md` all live there, all single-language Chinese, none of them part of the `{zh,en}` system. `docs/dev/design/_replaceability-map.md` used to be the one exception inside `docs/design/` — it moved out entirely, to `docs/ops/{zh,en}/_replaceability-map.md`, once it became clear the question it answers ("what can I swap out or customize, and what does that drag in") is exactly the kind of thing a real end user needs, alongside the deployment handbook and selection guide it now sits next to — not something that belongs inside the developer-only `dev/` tree at all. `docs/dev/design/_模板.md` is an authoring template for whoever writes the *next* design plan, not reference content anyone reads, so — like the 14 plans it's a template for — it stays a single file with no language pair.

**Why these specific documents stay single-language and consolidated under `dev/`**: `docs/dev/plans/`, `docs/dev/retrospectives/`, and `docs/dev/design/_调研记录/` (the long-form research dossiers behind a phase's design plans — a phase's design plans themselves condense the relevant findings, so this is genuinely a different, rarely-revisited document) are execution-process narrative — written once, cited by lookup rather than read wholesale as ongoing reference. `docs/dev/field-tested-pitfalls-log.md` is a historical record in substance too — each entry documents something that already happened and was already fixed — even though it's actively grep'd for pitfall codes; it briefly had an English pair, collapsed back to Chinese-only once it was recognized as belonging in this category rather than the AI-reads-it-in-full one. The 14 component design plans and `docs/dev/design/README.md` are read almost entirely by whoever is developing or extending a component — real end users have no reason to read them either. None of this content is judged worth the cost of full bilingual parity. **Historical records in any of these are never rewritten for a path change — only relocated if the file itself physically moves.**

⚠️ **A phase plan (`docs/dev/plans/0N-*.md`) gets a forward-looking exception, not a retroactive one.** Phase 1–3's plans stay Chinese-only permanently — they are never retroactively translated purely for consistency (their translation cost is large relative to how rarely a shipped phase's plan is reopened). But while a phase is still active, its plan is the literal day-by-day task checklist a session works from — a far higher re-read frequency than a retrospective or research dossier ever gets, even though it becomes purely historical the moment the phase ships, exactly like the others. Because of that, **a new phase's plan may be authored directly in English with a Chinese mirror from the moment it's created**, at the authoring session's discretion, without first needing the case this section makes for `docs/standards/`/`docs/ops/`. Research dossiers and retrospectives do **not** get this exception even when procedurally paired with a phase plan — see the previous paragraph.
