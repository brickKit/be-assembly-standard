...
⚠️ **"11" 是最初定下的固定编号——不是 `be-ops` 现在还在产出的项数。** 产出 4/7/8（全部三项都是合并部署专用）已经随着 brickKit 自己的 `servedBy` 机制陆续长出原生能力、一个个退休——每一行自己的说明见下方。编号本身保持不变、不重新排号，因为别的文档（包括本文档自己的 §14.1.2/§14.2.2）是按编号引用产出 9/10 的。
...
| 4 | ~~外壳合并配置：哪些组件进哪个外壳、端口分配、迁移执行顺序。~~ **已退休**——brickKit v0.4.2 新增原生 `BRICKKIT_SERVED_MEMBERS_CONFIG` 环境变量（数据来自注入引擎本来就已经给每个运行中组件算过一遍的结果），外壳启动器直接解析它，不再需要生成这一步 | 平台不提供合并部署支持（§13.6）——**写这一行时是真的，v0.4.2 之后不再成立** | **阶段四**，阶段四附加 Task 0.6 退休（`docs/plans/05a-迁移到servedBy.md`） |
...
| 7 | ~~每外壳的环境变量表：同外壳依赖 → `http://127.0.0.1:<端口>`；跨外壳/独立容器 → `http://<宿主机地址>:<端口>`~~ **已退休**——brickKit 原生的 `servedBy` 合并在生成阶段就直接把每一个 `*_ENDPOINT` 地址改写进外壳容器自己的环境变量 | 平台只往它自己生成的容器里注入，合并后那些容器不存在（§13.8）——**写这一行时是真的，`servedBy` 落地后不再成立** | **阶段四**，阶段四附加 Task 0.2/0.3 退休（`docs/plans/05a-迁移到servedBy.md`） |
| 8 | ~~shell-compose 的 `depends_on`：外壳之间的启动顺序~~ **已退休**——brickKit 原生的 `servedBy` 合并自己算外壳间启动顺序，它取代的那份手写 `infra/shell-compose.yml` 编排已删除 | 平台只排它生成的那些（§13.8）——**写这一行时是真的，`servedBy` 落地后不再成立** | **阶段四**，阶段四附加 Task 0.5 退休（`docs/plans/05a-迁移到servedBy.md`） |
...
**这条要解决的问题**：本项目自己定的规矩是**每一个改动**——哪怕是零行为/契约变更的纯文档修复或纯补测试——都要跳自己的版本号（本节整体的节奏；"为什么版本号和镜像必须绑在一起走，而不只是'为什么要跳版本号'"这条更深的理由见下文）。但一个组件的版本号一动，从来不会只影响它自己：每一个在 `dependencies.components` 里引用了它的别的组件，那一条引用也要跟着改（意味着那个组件自己也要跟着跳版本号，可能继续往下传）；根 `brickkit.yaml` 顶层 `components[].version` 那个 pin 要跟着改；根 `AGENTS.md`/`AGENTS.zh.md` 的组件名录表要跟着改；如果这次跳版本号的恰好是 `infra/authz` 或 `infra/iam-casdoor`，全项目每一个别的组件 `config:` 段里手写的 `authzBundleUrl`/`iamJwksUrl` 字面量（里面嵌着旧版本号拼出来的主机名）也要跟着改（踩坑记录 C18——曾经一次性漏改过全部 25 处，因为压根没有东西在替人盯着这件事）。这些全靠人一处一处 grep、一处一处手改，正是这个项目自己早就有名字的那类事情——"能机制化的判据必须机制化"（04-testing-standard.md §1）——只是这次要机制化的不是"检查"，是"传播"本身，此前一直没做只是因为"传播一处改动"跟"侦测一处漂移"是两种不同形状的活。
...
⚠️ **工具本身刻意不做的事**：`git commit`/`tag`/`make image`/`push`。批量改文件是安全、可审查、git 可撤销的操作；把一个打好标签、重建过的镜像推到好几个各自独立的组件仓库，正是这个项目一贯认为需要有把关在前的那类"对外可见、难以撤销"的动作（真机验证、`make gates`、看一眼 diff）——把这一步跟批量改文件塞进同一个程序里，意味着一条代码路径的 bug 会牵连另一条。所以流程刻意保持两阶段：
...
2. **按工具打印的顺序逐个组件收尾**（打印顺序是"根变更在前，牵连出来的在后"，纯粹是给人看的可读性——文件层面的版本号在第 1 步就已经全部定下来了，收尾这几步谁先谁后不影响结果）。对每一个：跑这个组件自己的测试、`git commit` + `git tag -a` + `make image`、`git push`（commit 和 tag 都推）。全部做完后提交装配仓库自己的改动（`brickkit.yaml`、两份 `AGENTS.md`、重新生成的 `.brickkit/manifests`），真机跑一遍 `brickkit up` 验证、`brickkit down`、推送。

**这个项目是从头到尾由 AI 开发的**，所以"有把关在前"不必等于"每次都要人亲手敲 git 命令"——它意味着**不管谁去执行** commit/tag/push 这一步，都要在每一次上继续做判断（这条 commit message 是不是真的说清楚了改了什么、这个级联看起来合不合理、这批改动是不是真的只是跳版本号、有没有真实的行为变更混进来），而不是让一个程序把这两类风险不经过任何判断地焊死在一起。完整的第 2 步流程——包括 commit message 怎么写、`bump-version` 算出来的级联什么时候看起来不对劲该停下核实而不是直接 apply、什么时候一批改动已经不再够格叫"只是跳版本号"、需要走正常开发流程的审查节奏——写成了 **`version-bump-ship` 技能**（`.claude/skills/version-bump-ship/`），配一个可选的发现步骤（`infra/scripts/list-unshipped-components.sh`——不确定这次会话到底碰过哪些组件时用，列出每个"HEAD 领先自己最新 tag"的组件仓库，跟 `make version-check` 拿来当门禁失败判据的是同一个信号，这里换个角度当"候选清单"读——列出来的不代表都该照单发布）。这个技能存在的意义是让这套节奏不用每次从头推导一遍，不是让它可以比写代码本身少一点判断力去执行。
...
- [ ] **B-13 单独 `brickkit up` 起来并跑六项验收**（见 [`01-阶段一`](../../dev/plans/01-阶段一-地基与第一块砖.md) §2 那张六项验收表。**每加一个组件都要重跑**，§9.6.1 档 4）
...
|---|---|---|---|---|---|
| **一** | 地基与第一块砖 | 1 | 环境能一键起来，第一块砖能独立活 | `mdm-customer` 六项验收全绿 | [`01-阶段一`](../../dev/plans/01-阶段一-地基与第一块砖.md) ✅ **已出档** |
| **二** | 验平台 | +4 = 5 | brickKit 的每条承诺都真的成立 | 设计书 §9.6.2 那 **20 条平台验收清单**逐条打勾 | [`02-阶段二`](../../dev/plans/02-阶段二-验平台.md) ✅ 已写，四份设计计划齐（未开工） |
