...
- **首次部署初始化**：建组织/应用/管理员、配好 OIDC 客户端与下面 §1.1 那几种登录方式，幂等可重跑。

### 1.1 本阶段要支持的登录方式（范围已定）

| 方式 | 说明 |
|---|---|
| **账号密码注册 + 登录** | 自助注册开放。**新注册账号零角色**——见下方 ⚠️ |
| **微信**（微信开放平台 · 网站应用） | 国内 |
| **QQ**（QQ 互联） | 国内 |
| **Google** | 海外 |

**这四种全部在 Casdoor 侧配置，本组件零代码**：不管用户用哪种方式认证成功，Casdoor 产出的都是同一个标准 OIDC 身份 token，适配层只认那个 token——**它根本不知道用户是扫码还是输密码进来的**。以后要加飞书/企业微信/LDAP/SAML，同样是 Casdoor 配置面的事，我们的 repo 一行不改。

⚠️ **社交登录必须配"零角色 + 绑定"，否则会漏账号**。社交身份（微信 openid / QQ openid / Google sub）**不天然对应"企业里的某个员工"**。所以：

1. 自助注册或社交首登产生的账号，**在 `infra-authz` 里没有任何角色**；
2. 它拿到的应用 token 里 `roles[]` 是空的 → 判定链走到内存 map 时并集为空 → **所有业务接口 403**；
3. 要能干活，必须由管理员在 `infra-authz` 的角色管理界面授角色。

**这条是 fail-safe 的**（陌生人登进来什么也做不了），但会积累空账号——所以管理侧要能看到"零角色账号"清单。⚠️ **不许为了"方便"把某个默认角色配成新用户自动获得**——那等于把注册入口变成提权入口。

⚠️ **登录方式里没有钉钉，这是一个有连带后果的决定**：Casdoor 因此拿不到用户的钉钉身份，而阶段三的审批通知要发钉钉。解法见 §9 第 3 条（走手机号匹配，不靠登录）。
...
⚠️ **明确决定：不留 claims 兜底缓存，签发这一步 fail-closed。** 曾经考虑过"authz 不可达时用上次的 claims 继续签"，判定为不做，三条理由：① 安全组件的默认应当是 fail-closed，"拿不到最新授权就照旧发"方向是反的；② 本项目的部署形态是**客户本地一台电脑**（§9.4），"机器好好的、13 个组件都活着、单单 authz 崩了"是很窄的场景，机器挂了的话谁签 token 都无所谓；③ 留了缓存就会重新出现"`stale_since` 已发布、而缓存还是旧角色"的时序窗口，把刚消掉的问题请回来。**要改这条之前先重读 §3.2 那张三个否决点的表。**
