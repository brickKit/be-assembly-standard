...
| 3 | ~~`access token` TTL 的最终取值~~ | ~~阶段三压测后~~ | ✅ **已定**：access **10 分钟**、refresh **12 小时 + 每次轮换、不滑动**。有了 `stale_since`，TTL 只决定「authz 不可达时已撤权的人还能撑多久」，所以取短；500 人在线 ≈ 0.83 QPS，代价可忽略。理由与「改它之前先想清楚改的是什么」见设计书 §14.1.6 |
| 4 | ~~字段级掩码怎么实现~~ | ~~阶段三~~ | ✅ **已定**：**手写，不进 `be-sdk`**。每个组件一个 `mask.go`，一个 DTO 一个函数，**置空不删字段**。只有 3 个组件 5 个字段，引入机制的成本超过重复成本；反射撞 protobuf 的 tag 问题，代码生成撞决策 32。守它的是**每个 `field` 键一条 L2 测试、同时覆盖单条与列表**——「单条掩了列表忘了」是这里唯一真会发生的错。见设计书 §14.1.10 |
