...
| 9 | `GetReservationStatusRequest` 只有 `reservation_id`（Task 8 契约），但"Reserve 超时"——防"薛定谔的超时"这个机制真正要防的场景——恰恰是调用方唯一拿不到 `reservation_id` 的场景（响应没收到，不代表请求没被处理并提交） | `erp-sales` Task 17 实现 TCC 补偿链时发现，本组件（`erp-inventory`）已经 v1.0.0 打标签发布 | **契约追加字段修复，不是重新设计**（决策 33 允许的向后兼容变更）：`GetReservationStatusRequest` 加 `idempotency_key`（field 2），`reservation_id` 留空时用它查。实现上白捡：`command_idempotency.idempotency_key → result_id`（这里 `result_id` 就是 `Reserve` 写入的 `reservation_id` 本身）这张映射早就存在（claim-first 幂等的副产品），`GetReservationStatus` 只是多了一步"先用 `idempotency_key` 查 `command_idempotency` 解出 `reservation_id`，查不到就是真正的 NOT_FOUND"。**NOT_FOUND 在这里的含义是可以安全重试 `Reserve`**——`command_idempotency` 里的映射只在 `Reserve` 事务提交后才对其他事务可见（READ COMMITTED），查不到就意味着提交还没发生（或从未发生），claim-first 幂等保证带同一个 `idempotency_key` 重试不会产生重复预留。`v1.0.0 → v1.0.1`，纯新增字段，`buf breaking` 验证过不算破坏性变更。⚠️ **`v1.0.1` 发布后立刻发现漏了一半**：`GetReservationStatusResponse` 只回 `status`/`order_id`，调用方按 `idempotency_key` 查到 `RESERVED` 却还是拿不到**真正的** `reservation_id`——后续 `CancelOrder`/`ShipOrder` 要用它调 `CancelReservation`/`ConfirmIssue`，只知道状态不知道 id 等于没解决问题。同一批追加 `GetReservationStatusResponse.reservation_id`（field 3），`v1.0.1 → v1.0.2`，同样是纯新增字段。**教训**：改一个"查状态"接口时，要连着检查它的返回值是否带全了调用方后续步骤真正需要的东西，不能只看请求侧缺什么 |
