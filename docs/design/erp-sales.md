...
| `crm.opportunity.won.v1` | `crm-opportunity`（**阶段三**） | 赢单自动转订单（附录 E，✅ **Task 14 已落地**）：`backend/internal/tcc/opportunity_won.go` 的 `HandleOpportunityWon` 复用同一套建单+确认逻辑（`createOrderForOpportunity`/`confirmOrderForOpportunity`，与 `CreateOrder`/`ConfirmOrder` 共享 `o.reserve`/`o.compensateReserve` 等核心 helper），唯一区别是客户端走系统身份（`client.*System`，事件 handler 语境没有 JWT 可透传，§14.2.6）、归属字段（`owner_id`/`dept_path`）取自事件本身而不是 `besdk.ScopeOf(ctx)` |

⚠️ **`crm.opportunity.won.v1` 任何一步失败都不通过事件回传给 `crm-opportunity`**（§9 待决问题 2 已定）：建一条 `infra-workflow` exception 待办，`assignee_sub` 用事件里的 `owner_id`——这条待办的 `source_aggregate` 是 `"opportunity_conversion"`，与既有"补偿连续失败 3 次"那条异常待办的 `source_aggregate`（`"sales_order"`）是两个独立的来源标识，`workflowTaskCompletedHandler` 目前只处理后者，处理前者不会触发任何自动恢复动作——人工介入的结果（比如库存补货后手动重新走一遍下单流程）目前完全在系统之外，没有自动重试机制，这是刻意的（"尽力而为"，见 `opportunity_won.go` 顶部注释），不是遗漏。
